    ...
    CScript ToScript(const Ctx& ctx) const
    {
        ...
        auto upfn = [&ctx, is_tapscript](bool verify, const Node& node, std::span<CScript> subs) -> CScript {
            switch (node.fragment) {
                ...
                case Fragment::WRAP_C: return BuildScript(std::move(subs[0]), verify ? OP_PQCHECKSIGVERIFY : OP_PQCHECKSIG);
                ...
                case Fragment::MULTI_A: {
                    CHECK_NONFATAL(is_tapscript);
                    CScript script = BuildScript(ctx.ToPKBytes(*node.keys.begin()), OP_PQCHECKSIG);
                    for (auto it = node.keys.begin() + 1; it != node.keys.end(); ++it) {
                        script = BuildScript(std::move(script), ctx.ToPKBytes(*it), OP_PQCHECKSIGADD);
                    }
        ...
        case DecodeContext::SINGLE_BKV_EXPR: {
            ...
            if (last - in >= 4 && in[0].first == OP_NUMEQUAL) {
                ...
                for (int pos = 2;; pos += 2) {
                    ...
                    if (in[pos].first != OP_PQCHECKSIGADD && in[pos].first != OP_PQCHECKSIG) return {};
                    ...
                    if (in[pos].first == OP_PQCHECKSIG) break;
                }
