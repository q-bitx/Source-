...
bool HidingSigningProvider::GetDilithiumPubKey(const DilithiumPKHash& address, CDilithiumPubKey& pubkey) const
{
    return m_provider->GetDilithiumPubKey(address, pubkey);
}

bool HidingSigningProvider::GetDilithiumKeyByHash(const DilithiumPKHash& address, std::vector<unsigned char>& key_bytes) const
{
    if (m_hide_secret) return false;
    return m_provider->GetDilithiumKeyByHash(address, key_bytes);
}
...
bool FlatSigningProvider::GetDilithiumPubKey(const DilithiumPKHash& address, CDilithiumPubKey& pubkey) const
{
    return LookupHelper(dilithium_pubkeys, address, pubkey);
}

bool FlatSigningProvider::GetDilithiumKeyByHash(const DilithiumPKHash& address, std::vector<unsigned char>& key_bytes) const
{
    return LookupHelper(dilithium_keys, address, key_bytes);
}
...
bool MultiSigningProvider::GetDilithiumPubKey(const DilithiumPKHash& address, CDilithiumPubKey& pubkey) const
{
    for (const auto& provider: m_providers) {
        if (provider->GetDilithiumPubKey(address, pubkey)) return true;
    }
    return false;
}

bool MultiSigningProvider::GetDilithiumKeyByHash(const DilithiumPKHash& address, std::vector<unsigned char>& key_bytes) const
{
    for (const auto& provider: m_providers) {
        if (provider->GetDilithiumKeyByHash(address, key_bytes)) return true;
    }
    return false;
}
