...
static bool EvalPQChecksigPreTapscript(
    ...
    bool& success)
{
    ...
    // --- CONSENSUS: hard-fail on invalid PQ key/sig sizes (like STRICTENC) ---
...
// If a specific error was set (SIG_DER, PUBKEYTYPE, etc), don't override it.
if (!success && (flags & SCRIPT_VERIFY_NULLFAIL) && !vchSig.empty() &&
    (serror == nullptr || *serror == SCRIPT_ERR_OK)) {
    return set_error(serror, SCRIPT_ERR_SIG_NULLFAIL);
}
    return true;
...
/** Helper for OP_PQCHECKSIG, OP_PQCHECKSIGVERIFY, and (in Tapscript) OP_PQCHECKSIGADD.
 *
    ...
    try
    {
        for (; pc < pend; ++opcode_pos) {
            ...
            switch (opcode)
            {
                ...
                case OP_QBITX:
                    // Q-BitX marker opcode: semantic NOP, no operation
                    break;
                ...
                case OP_CHECKSIGDILITHIUMVERIFY:
                {
                    ...
                    // Drop the signature in pre-segwit scripts but not segwit scripts
                    // This mirrors OP_CHECKSIG behavior
                    if (sigversion == SigVersion::BASE) {
                        int found = FindAndDelete(scriptCode, CScript() << vchSig);
                        if (found > 0 && (flags & SCRIPT_VERIFY_CONST_SCRIPTCODE))
                            return set_error(serror, SCRIPT_ERR_SIG_FINDANDDELETE);
                ...
                case OP_CHECKMULTISIGVERIFY:
                {
                    ...
                    if ((flags & SCRIPT_VERIFY_DISCOURAGE_UPGRADABLE_NOPS) && sigversion == SigVersion::BASE) {
                        return set_error(serror, SCRIPT_ERR_DISCOURAGE_UPGRADABLE_NOPS);
                    }
                ...
                {
                    // ([sig ...] num_of_signatures [pubkey ...] num_of_pubkeys -- bool)
                    // Stack contract: <dummy> <sig1> ... <sigm> <m> <pub1> ... <pubn> <n> OP_CHECKMULTISIGDILITHIUM
                    ...
                    // ===== ANTI-DoS: Validate ALL pubkeys BEFORE any expensive verification =====
                    for (int k = 0; k < nKeysCount; k++) {
                        valtype& vchPubKey = stacktop(-ikey - k);
                        // Size check (consensus)
                        if (vchPubKey.size() > MAX_SCRIPT_ELEMENT_SIZE) {
                            return set_error(serror, SCRIPT_ERR_PUBKEYTYPE);
                        }
                        // PQ-specific size check
                        if (vchPubKey.size() != DILITHIUM_PUBLICKEYBYTES) {
                            return set_error(serror, SCRIPT_ERR_PUBKEYTYPE);
                        }
                        // Format sanity check: reject all-zero pubkeys
                        bool all_zero = true;
                        for (unsigned char c : vchPubKey) {
                            if (c != 0) {
                                all_zero = false;
                                break;
                            }
                    ...
                    // ===== ANTI-DoS: Validate ALL signatures BEFORE any expensive verification =====
                    for (int k = 0; k < nSigsCount; k++) {
                        valtype& vchSig = stacktop(-isig - k);
                        // Size check (consensus)
                        if (vchSig.size() > MAX_SCRIPT_ELEMENT_SIZE) {
                            return set_error(serror, SCRIPT_ERR_SIG_DER);
                        }
                        // Empty signatures are allowed (for multisig)
                        if (vchSig.empty()) {
                            continue;
                        }
                        // PQ-specific size check: must be DILITHIUM_SIGNATUREBYTES + 1 (hashtype byte)
                        if (vchSig.size() != DILITHIUM_SIGNATUREBYTES + 1) {
                            return set_error(serror, SCRIPT_ERR_SIG_DER);
                        }
                        ...
                        bool all_zero = true;
                        for (size_t j = 0; j < vchSig.size() - 1; j++) {  // Exclude hashtype byte
                            if (vchSig[j] != 0) {
                                all_zero = false;
                                break;
                            }
                    ...
                    // Matching algorithm: iterate pubkeys left-to-right, advance signature index only on success
                    bool fSuccess = true;
                    while (fSuccess && nSigsCount > 0)
                    {
                        valtype& vchSig    = stacktop(-isig);
...
case OP_PQCHECKSIGADD:
{
    if (stack.size() < 3) return set_error(serror, SCRIPT_ERR_INVALID_STACK_OPERATION);

    // Stack: [num, sig, pubkey] (top to bottom: pubkey, sig, num)
    valtype vchPubKey = stacktop(-1);
    valtype vchSig    = stacktop(-2);
    valtype vchNum    = stacktop(-3);

    // Only support BASE and WITNESS_V0 (same as other PQ opcodes)
    ...
    // Use existing PQ verification path
    if (!EvalPQChecksigPreTapscript(
            vchSig,
            vchPubKey,
            pbegincodehash,
    ...
    // Parse num
    CScriptNum num(vchNum, fRequireMinimal);
    if (fSuccess) {
        num += bnOne;
    }
...
/*
                case OP_PQCHECKMULTISIG:
                case OP_PQCHECKMULTISIGVERIFY:
                {
                    ...
                    if (opcode == OP_PQCHECKMULTISIGVERIFY)
                    {
                        if (fSuccess)
                            popstack(stack);
                        else
...
{
    // 1) Pubkey size must be exact for Dilithium
    if (vchPubKey.size() != DILITHIUM_PUBLICKEYBYTES) {
        return set_error(serror, SCRIPT_ERR_PUBKEYTYPE);
    }

    // 2) Signature must be exactly: <dilithium_sig (fixed)> <hashtype (1 byte)>
    if (vchSigIn.size() != DILITHIUM_SIGNATUREBYTES + 1) {
        return set_error(serror, SCRIPT_ERR_SIG_DER);
    }

    // Split hashtype from signature
    const unsigned char nHashType = vchSigIn.back();
    std::vector<unsigned char> sig_no_hashtype(vchSigIn.begin(), vchSigIn.end() - 1);

    if (sig_no_hashtype.size() != DILITHIUM_SIGNATUREBYTES) {
        return set_error(serror, SCRIPT_ERR_SIG_DER);
    }

    // ---- MVP hashtype policy: ONLY SIGHASH_ALL (0x01) ----
    if (nHashType != 0x01) {
        return set_error(serror, SCRIPT_ERR_SIG_HASHTYPE);
    }

    // ---- Q-BitX DoS hardening (cheap rejects before PQ_Verify) ----
    auto is_all_zero = [](const std::vector<unsigned char>& v) {
        for (unsigned char c : v) if (c != 0) return false;
        return true;
    };

    const unsigned char baseType = nHashType & 0x1f; // SIGHASH type mask
if (baseType < SIGHASH_ALL || baseType > SIGHASH_SINGLE) {
    return set_error(serror, SCRIPT_ERR_SIG_HASHTYPE);
}
if (nHashType & ~(SIGHASH_ALL | SIGHASH_NONE | SIGHASH_SINGLE | SIGHASH_ANYONECANPAY)) {
}

    // Validate hashtype byte early (keep it strict for MVP)
    // Conservative: disallow unknown bits. (Adjust if you explicitly support them.)
    if ((nHashType & ~0x1f) != 0 && (nHashType & 0x80) == 0) {
        return set_error(serror, SCRIPT_ERR_SIG_HASHTYPE);
    }

    // Reject trivially bad pubkeys/sigs early (avoid expensive SignatureHash/PQ_Verify on junk)
    if (is_all_zero(vchPubKey) || is_all_zero(sig_no_hashtype)) {
        return set_error(serror, SCRIPT_ERR_SIG_DER);
    }

    // segwit v0 requires amount
    if (sigversion == SigVersion::WITNESS_V0 && amount < 0) {
        return HandleMissingData(m_mdb);
    }

    // For now: we do not support Tapscript/Taproot PQ checksig
    if (sigversion == SigVersion::TAPSCRIPT || sigversion == SigVersion::TAPROOT) {
        return set_error(serror, SCRIPT_ERR_BAD_OPCODE);
    }

    // Compute consensus sighash
    ...
    // IMPORTANT: if cryptographic verification fails, that's not a "format" error
...
// Explicit template instantiations for signature checkers used by unit tests and node code.
template class GenericTransactionSignatureChecker<CTransaction>;
template class GenericTransactionSignatureChecker<const CTransaction>;
template class GenericTransactionSignatureChecker<CMutableTransaction>;
