...
static const unsigned int MAX_SCRIPT_ELEMENT_SIZE = 8192 ;
...
static constexpr opcodetype OP_QBITX = (opcodetype)0xc0;
...
//static constexpr opcodetype OP_PQCHECKMULTISIG = (opcodetype)0xc4;
...
//{
  ...
  //  int cnt{0};
    ...
    //        }
       ...
       // }
  //      } else {
   	 // Cast to int64_t to resolve ambiguity
//	    ret << static_cast<int64_t>(input);
//	}
//	 cnt++;
...
CScript BuildScript(Ts&&... inputs)
{
    ...
    ([&ret, &cnt](auto&& input) {
        using InputType = std::remove_cv_t<std::remove_reference_t<decltype(input)>>;

        if constexpr (std::is_same_v<InputType, CScript>) {
            if (cnt == 0) {
                ret = std::forward<decltype(input)>(input);
            } else {
                ret.insert(ret.end(), input.begin(), input.end());
            }
        } else if constexpr (std::is_same_v<InputType, std::vector<unsigned char>>) {
            ret.insert(ret.end(), input.begin(), input.end());
        } else if constexpr (std::is_integral_v<InputType> && !std::is_same_v<InputType, bool>) {
            ret << static_cast<int64_t>(input);
        } else {
            ret << input;
        }
