...
size_t ComputeScriptLen(Fragment fragment, Type sub0typ, size_t subsize, uint32_t k, size_t n_subs,
    ...
    switch (fragment) {
        ...
        case Fragment::OLDER:
	case Fragment::AFTER: return 1 + BuildScript(static_cast<int64_t>(k)).size();
...
std::optional<std::vector<Opcode>> DecomposeScript(const CScript& script)
{
    ...
    while (it != itend) {
        ...
        } else if (opcode == OP_PQCHECKSIGVERIFY) {
            // Decompose OP_CHECKSIGVERIFY into OP_CHECKSIG OP_VERIFY
            out.emplace_back(OP_PQCHECKSIG, std::vector<unsigned char>());
            opcode = OP_VERIFY;
        ...
        } else if (it != itend && (opcode == OP_PQCHECKSIG || opcode == OP_CHECKMULTISIG || opcode == OP_EQUAL || opcode == OP_NUMEQUAL) && (*it == OP_VERIFY)) {
