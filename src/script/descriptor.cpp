...
protected:
    ...
    {
        if (m_xonly) {
            CScript script = CScript() << ToByteVector(XOnlyPubKey(keys[0])) << OP_PQCHECKSIG;
...
protected:
    ...
    std::vector<CScript> MakeScripts(const std::vector<CPubKey>& keys, std::span<const CScript>, FlatSigningProvider&) const override {
        ...
        ret << ToByteVector(xkeys[0]) << OP_PQCHECKSIG;
        for (size_t i = 1; i < keys.size(); ++i) {
            ret << ToByteVector(xkeys[i]) << OP_PQCHECKSIGADD;
        }
//        ret << m_threshold << OP_NUMEQUAL;
//       return Vector(std::move(ret));
	ret << static_cast<int64_t>(m_threshold) << OP_NUMEQUAL;
	return Vector(std::move(ret));
    }
public:
...
{
    if (ctx == ParseScriptContext::P2TR && script.size() == 34 && script[0] == 32 && script[33] == OP_PQCHECKSIG) {
    ...
    if (txntype == TxoutType::DILITHIUM_PUBKEYHASH && (ctx == ParseScriptContext::TOP || ctx == ParseScriptContext::P2SH)) {
        // For Dilithium P2PKH, extract destination and return AddressDescriptor
        CTxDestination dest;
  ...
  //     if (txntype == TxoutType::WITNESS_V1_TAPROOT && ctx == ParseScriptContext::TOP) {
	{
	 if (data.empty() || data[0].size() != 32) return nullptr;
	 XOnlyPubKey pubkey;
