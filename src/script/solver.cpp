...
std::string GetTxnOutputType(TxoutType t)
{
    ...
    case TxoutType::QBITX_DILITHIUM: return "qbitx_dilithium";
...
static bool MatchPayToPubkey(const CScript& script, valtype& pubkey)
{
    ...
    // Uncompressed: 65 bytes + 1 byte push + 1 byte OP_CHECKSIG = 67 bytes
    if (script.size() == CPubKey::SIZE + 2 && script[0] == CPubKey::SIZE && script.back() == OP_CHECKSIG) {
        pubkey = valtype(script.begin() + 1, script.begin() + CPubKey::SIZE + 1);
        return CPubKey::ValidSize(pubkey);
    }
    // Compressed: 33 bytes + 1 byte push + 1 byte OP_CHECKSIG = 35 bytes
...
// Match Dilithium P2PK: <pubkey_bytes> OP_CHECKSIGDILITHIUM
static bool MatchPayToDilithiumPubkey(const CScript& script, valtype& pubkey)
{
    ...
    // First opcode must push the pubkey
    if (!script.GetOp(it, opcode, data)) return false;
    if (!IsPushdataOp(opcode)) return false;
    if (data.size() != DILITHIUM_PUBLICKEYBYTES) return false;
    
    // Next opcode must be OP_CHECKSIGDILITHIUM
    if (!script.GetOp(it, opcode)) return false;
    if (opcode != OP_CHECKSIGDILITHIUM) return false;
...
// MVP Q-BitX: OP_QBITX <push(dilithium_pubkey)> OP_PQCHECKSIG
static bool MatchQbitxDilithium(const CScript& script, valtype& pqpub)
{
    pqpub.clear();
    ...
    // 1) OP_QBITX
    if (!script.GetOp(it, opcode) || opcode != OP_QBITX) return false;

    if (!script.GetOp(it, opcode, data)) return false;
    if (data.size() != DILITHIUM_PUBLICKEYBYTES) return false;

    // 3) OP_PQCHECKSIG
    if (!script.GetOp(it, opcode) || opcode != OP_PQCHECKSIG) return false;
...
static bool MatchPayToQbitxDilithium(const CScript& script, valtype& dilithium_pubkey)
{
    // OP_QBITX <dilithium_pubkey> OP_PQCHECKSIG
    if (script.size() < 3) return false;
    if (script[0] != OP_QBITX) return false;
    if (script.back() != OP_PQCHECKSIG) return false;
    ...
    // consume OP_QBITX
    if (!script.GetOp(it, opcode, data)) return false;
    if (opcode != OP_QBITX) return false;

    // next must push pubkey
    if (!script.GetOp(it, opcode, data)) return false;
    if (!IsPushdataOp(opcode)) return false;
    if (data.size() != DILITHIUM_PUBLICKEYBYTES) return false;

    // last opcode must be OP_PQCHECKSIG and iterator should end there
    if (!script.GetOp(it, opcode, data)) return false;
    if (opcode != OP_PQCHECKSIG) return false;
    if (it != script.end()) return false;

    dilithium_pubkey = std::move(data);
    return true;
}
...
static bool MatchDilithiumMultisig(const CScript& script, int& required_sigs, std::vector<valtype>& pubkeys)
{
    ...
    if (!req_sigs) return false;
    required_sigs = *req_sigs;
    while (script.GetOp(it, opcode, data) && data.size() == DILITHIUM_PUBLICKEYBYTES) {
        pubkeys.emplace_back(std::move(data));
    }
    auto num_keys = GetScriptNumber(opcode, data, required_sigs, MAX_PUBKEYS_PER_MULTISIG);
    if (!num_keys) return false;
...
// Q-BitX Dilithium output detection (non-witness, MVP)
// OP_QBITX <DILITHIUM_PUBLICKEYBYTES> <pubkey> OP_PQCHECKSIG
valtype qbitx_pub;
if (MatchQbitxDilithium(scriptPubKey, qbitx_pub)) {
    vSolutionsRet.push_back(std::move(qbitx_pub));
    return TxoutType::QBITX_DILITHIUM;
}
// Q-BitX Dilithium output detection

    // Q-BitX Dilithium script patterns (check before regular patterns)
    valtype dilithium_data;
    if (MatchPayToDilithiumPubkey(scriptPubKey, dilithium_data)) {
        vSolutionsRet.push_back(std::move(dilithium_data));
        return TxoutType::DILITHIUM_PUBKEY;
    }
    if (MatchPayToDilithiumPubkeyHash(scriptPubKey, dilithium_data)) {
        vSolutionsRet.push_back(std::move(dilithium_data));
...
{
    return CScript() << std::vector<unsigned char>(pubKey.begin(), pubKey.end()) << OP_PQCHECKSIG;
}
...
//{
   ...
   // script << keys.size() << OP_PQCHECKMULTISIG;
...
//}
{
    ...
    script << static_cast<int64_t>(nRequired);
    for (const CPubKey& key : keys)
        script << ToByteVector(key);
    script << static_cast<int64_t>(keys.size()) << OP_CHECKMULTISIG;

    return script;
}

CScript GetScriptForDilithiumMultisig(int nRequired, const std::vector<std::vector<unsigned char>>& keys)
{
    CScript script;
    ...
    for (const std::vector<unsigned char>& key : keys)
        script << key;
    script << static_cast<int64_t>(keys.size()) << OP_CHECKMULTISIGDILITHIUM;
