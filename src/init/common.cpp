...
bool StartLogging(const ArgsManager& args)
{
    ...
    if (!LogInstance().StartLogging()) {
            return InitError(Untranslated(strprintf("Could not open debug log file %s",
                PathToString(LogInstance().m_file_path))));
    }

    if (!LogInstance().m_log_timestamps)
        LogPrintf("Startup time: %s\n", FormatISO8601DateTime(GetTime()));
    LogPrintf("Default data directory %s\n", PathToString(GetDefaultDataDir()));
    LogPrintf("Using data directory %s\n", PathToString(gArgs.GetDataDirNet()));

    fs::path config_file_path = args.GetConfigFilePath();
    LogPrintf("Using config file %s\n", PathToString(config_file_path));

    // Only log conf file usage message if conf file actually exists.
    if (args.IsArgNegated("-conf")) {
        LogInfo("Config file: <disabled>");
    } else if (fs::is_directory(config_file_path)) {
        LogWarning("Config file: %s (is directory, not file)", PathToString(config_file_path));
    } else if (fs::exists(config_file_path)) {
        LogPrintf("Config file: %s\n", PathToString(config_file_path));
    } else if (args.IsArgSet("-conf")) {
        InitWarning(strprintf(_("The specified config file %s does not exist"), PathToString(config_file_path)));
    } else {
...
void LogPackageVersion()
{
   ...
   // LogPrintf(CLIENT_NAME " version %s\n", version_string);
    LogPrintf("%s version %s\n", CLIENT_NAME.c_str(), version_string.c_str());
}
} // namespace init
