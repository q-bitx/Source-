...
LockResult LockDirectory(const fs::path& directory, const fs::path& lockfile_name, bool probe_only)
{
    ...
    if (dir_locks.count(PathToString(pathLockFile))) {
    ...
    if (!lock->TryLock()) {
        LogError("Error while attempting to lock directory %s: %s\n", PathToString(directory), lock->GetReason());
        return LockResult::ErrorLock;
    }
    if (!probe_only) {
        // Lock successful and we're not just probing, put it into the map
        dir_locks.emplace(PathToString(pathLockFile), std::move(lock));
    }
    return LockResult::Success;
}
} // namespace util
...
{
    LOCK(cs_dir_locks);
    dir_locks.erase(PathToString(directory / lockfile_name));
}
