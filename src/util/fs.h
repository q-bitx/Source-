...
static inline fs::path u8path(const std::string& utf8_str)
{
...
static inline fs::path absolute(const fs::path& p)
{
...
//template<typename T> static inline fs::path operator/(path p1, T p2) = delete;
//template<typename T> static inline fs::path operator+(path p1, T p2) = delete;

// Disallow implicit std::string conversion for copy_file
// to avoid locale-dependent encoding on Windows.
static inline bool copy_file(const fs::path& from, const fs::path& to, fs::copy_options options)
{
 ...
 * Convert byte string to path object. Inverse of \ref PathToString.
 */
static inline fs::path PathFromString(const std::string& string)
{
#ifdef WIN32
...
//}
static inline bool create_directories(const fs::path& p)
{
...
bool create_directories(const fs::path& p, std::error_code& ec) = delete;
 // namespace fs
