...
CDBWrapper::CDBWrapper(const DBParams& params)
    : m_db_context{std::make_unique<LevelDBContext>()}, m_name{PathToString(params.path.stem())}, m_path{params.path}, m_is_memory{params.memory_only}
{
    ...
    if (params.memory_only) {
        ...
        if (params.wipe_data) {
            LogPrintf("Wiping LevelDB in %s\n", PathToString(params.path));
            leveldb::Status result = leveldb::DestroyDB(PathToString(params.path), DBContext().options);
            HandleError(result);
        }
    ...
    leveldb::Status status = leveldb::DB::Open(DBContext().options, PathToString(params.path), &DBContext().pdb);
    HandleError(status);
    ...
    if (!key_exists && params.obfuscate && IsEmpty()) {
        ...
        LogPrintf("Wrote new obfuscate key for %s: %s\n", PathToString(params.path), HexStr(obfuscate_key));
    }
