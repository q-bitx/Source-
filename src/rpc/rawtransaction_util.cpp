...
{
    // Check if outputs_in is an array with objects containing "scriptPubKey" field
    bool has_scriptpubkey_format = false;
    if (outputs_in.isArray()) {
        for (size_t i = 0; i < outputs_in.size(); ++i) {
            const UniValue& output = outputs_in[i];
            if (output.isObject() && output.exists("scriptPubKey")) {
                has_scriptpubkey_format = true;
                break;
            }
        }
    }

    if (has_scriptpubkey_format) {
        // New format: array of objects with "scriptPubKey" and "amount" fields
        for (size_t i = 0; i < outputs_in.size(); ++i) {
            const UniValue& output = outputs_in[i];
            if (!output.isObject()) {
                throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, output must be an object");
            }

            RPCTypeCheckObj(output,
                {
                    {"scriptPubKey", UniValueType(UniValue::VSTR)},
                    {"amount", UniValueType()}, // Can be NUM or STR
                },
                true, false);

            if (!output.exists("scriptPubKey")) {
                throw JSONRPCError(RPC_INVALID_PARAMETER, "Missing scriptPubKey field");
            }
            if (!output.exists("amount")) {
                throw JSONRPCError(RPC_INVALID_PARAMETER, "Missing amount field");
            }

            // Parse scriptPubKey hex
            std::vector<unsigned char> scriptBytes = ParseHexV(output["scriptPubKey"], "scriptPubKey");
            CScript scriptPubKey(scriptBytes.begin(), scriptBytes.end());

            // Parse amount
            CAmount nAmount = AmountFromValue(output["amount"]);

            CTxOut out(nAmount, scriptPubKey);
            rawTx.vout.push_back(out);
        }
    } else {
        // Legacy format: object with address/data keys or array of key-value pairs
