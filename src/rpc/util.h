...
// Minimal concrete FillableSigningProvider for RPC multisig operations
class RPCFillableSigningProvider : public FillableSigningProvider
{
public:
    RPCFillableSigningProvider() = default;
    
    // Helper to iterate over scripts (for compatibility with FlatSigningProvider usage)
    std::map<CScriptID, CScript> GetScriptsMap() const
    {
        LOCK(cs_KeyStore);
        return mapScripts;
    }
