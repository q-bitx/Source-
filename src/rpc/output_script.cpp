...
static RPCHelpMan createmultisig()
{
    return RPCHelpMan{"createmultisig",
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            ...
            RPCFillableSigningProvider keystore;
            CScript inner;
            const CTxDestination dest = AddAndGetMultisigDestination(required, pubkeys, output_type, keystore, inner);

            // Make the descriptor
            FlatSigningProvider flat_provider;
            auto scripts_map = keystore.GetScriptsMap();
            for (const auto& [id, script] : scripts_map) {
                flat_provider.scripts.emplace(id, script);
            }
            std::unique_ptr<Descriptor> descriptor = InferDescriptor(GetScriptForDestination(dest), flat_provider);
