...
static RPCHelpMan getblocksubsidy()
{
    return RPCHelpMan{
        "getblocksubsidy",
        "Returns the block subsidy (block reward) for a given height.\n",
        {
            {"height", RPCArg::Type::NUM, RPCArg::Optional::NO, "Block height"},
        },
        RPCResult{
            RPCResult::Type::STR_AMOUNT, "subsidy", "The block subsidy at this height"
        },
        RPCExamples{
            HelpExampleCli("getblocksubsidy", "0") +
            HelpExampleCli("getblocksubsidy", "840000")
        },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            const int height = request.params[0].getInt<int>();
            if (height < 0) {
                throw JSONRPCError(RPC_INVALID_PARAMETER, "height must be >= 0");
            }
            const CAmount subsidy = GetBlockSubsidy(height, Params().GetConsensus());
            return ValueFromAmount(subsidy);
        }
    };
}
...
static RPCHelpMan waitfornewblock()
{
    return RPCHelpMan{"waitfornewblock",
                ...
                "\nMake sure to use no RPC timeout (qbitx-cli -rpcclienttimeout=0)",
                {
    ...
    if (action == "status") {
        ...
        for (const UniValue& scanobject : request.params[1].get_array().getValues()) {
            ...
            for (CScript& script : scripts) {
                std::unique_ptr<Descriptor> inferred_desc = InferDescriptor(script, provider);
                std::string inferred = inferred_desc ? inferred_desc->ToString()
                                                     : strprintf("raw(%s)", HexStr(script));
...
static RPCHelpMan scanblocks()
{
    return RPCHelpMan{"scanblocks",
        ...
        "This call may take several minutes. Make sure to use no RPC timeout (qbitx-cli -rpcclienttimeout=0)",
        {
            scan_action_arg_desc,
            scan_objects_arg_desc,
...
static RPCHelpMan getdescriptoractivity()
{
    return RPCHelpMan{"getdescriptoractivity",
        ...
        "This call may take several minutes. If you encounter timeouts, try specifying no RPC timeout (qbitx-cli -rpcclienttimeout=0)",
        {
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (afile.IsNull()) {
        throw JSONRPCError(
            RPC_INVALID_PARAMETER,
            "Couldn't open file " + temppath.string() + " for writing.");
    }
    ...
    const std::function<void()>& interruption_point)
{
    LOG_TIME_SECONDS(strprintf("writing UTXO snapshot at height %s (%s) to file %s (via %s)",
        ...
        PathToString(path), PathToString(temppath)));
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (!activation_result) {
        throw JSONRPCError(RPC_INTERNAL_ERROR, strprintf("Unable to load UTXO snapshot: %s. (%s)", util::ErrorString(activation_result).original, path.string()));
    }
    ...
    result.pushKV("path", PathToString(path));
    return result;
},
    };
}
