...
// Copyright (c) 2022-2025 The Q-BitX Core developers
...
//{
   // static const std::string RESULT(CLIENT_NAME)
 //" stopping";
    ...
    // to the client (intended for testing)
            //    "\nRequest a graceful shutdown of " CLIENT_NAME ".",
              ...
              //  RPCResult{RPCResult::Type::STR, "", "A string with the content '" + RESULT + "'"},
                //RPCResult{RPCResult::Type::STR, "", std::string("A string with the content '") + RESULT + "'"},
 		//RPCExamples{""},
       // [&](const RPCHelpMan& self, const JSONRPCRequest& jsonRequest) -> UniValue
//{
...
static RPCHelpMan stop()
{
    static const std::string result = std::string(CLIENT_NAME) + " stopping";
    ...
    return RPCHelpMan(
        ...
        RPCExamples{
            std::string(HelpExampleCli("stop", "") + HelpExampleRpc("stop", ""))
        },
        [&](const RPCHelpMan& self, const JSONRPCRequest& jsonRequest) -> UniValue {
            CHECK_NONFATAL((CHECK_NONFATAL(EnsureAnyNodeContext(jsonRequest.context).shutdown_request))());
            if (!jsonRequest.params.empty() && jsonRequest.params[0].isNum()) {
                UninterruptibleSleep(std::chrono::milliseconds{jsonRequest.params[0].getInt<int>()});
            }
...
static RPCHelpMan uptime()
{
    return RPCHelpMan(
        ...
        {},
        RPCResults{
            RPCResult{RPCResult::Type::NUM, "", "The number of seconds that the server has been running"}
        },
        RPCExamples{
            std::string(HelpExampleCli("uptime", "") + HelpExampleRpc("uptime", ""))
        },
        [](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
            return GetTime() - GetStartupTime();
        }
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
   ...
   // const std::string path = LogInstance().m_file_path.u8string(); // utf8string //u8string
