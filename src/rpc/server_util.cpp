...
{
    // Lazy initialization for regtest: create mining interface if not already present
    static std::once_flag init_flag;
    if (!node.mining) {
        std::call_once(init_flag, [&]() {
            // Remove const to allow initialization (node.mining is mutable for lazy init)
            NodeContext& mutable_node = const_cast<NodeContext&>(node);
            mutable_node.mining = interfaces::MakeMining(mutable_node);
        });
