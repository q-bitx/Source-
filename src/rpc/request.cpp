...
bool GenerateAuthCookie(std::string* cookie_out, std::optional<fs::perms> cookie_perms)
{
    ...
    if (!file.is_open()) {
        LogWarning("Unable to open cookie authentication file %s for writing", PathToString(filepath_tmp));
        return false;
    }
    file << cookie;
    file.close();
    ...
    if (!RenameOver(filepath_tmp, filepath)) {
        LogWarning("Unable to rename cookie authentication file %s to %s", PathToString(filepath_tmp), PathToString(filepath));
        return false;
    }
    if (cookie_perms) {
        std::error_code code;
        ...
        if (code) {
            LogWarning("Unable to set permissions on cookie authentication file %s", PathToString(filepath));
            return false;
        }
    }
