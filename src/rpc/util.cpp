...
{
    return "> qbitx-cli " + methodname + " " + args + "\n";
}

std::string HelpExampleCliNamed(const std::string& methodname, const RPCArgList& args)
{
    std::string result = "> qbitx-cli -named " + methodname;
...
std::string HelpExampleRpc(const std::string& methodname, const std::string& args)
{
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"2.0\", \"id\": \"curltest\", "
        "\"method\": \"" + methodname + "\", \"params\": [" + args + "]}' -H 'content-type: application/json' http://127.0.0.1/\n";
}
...
std::string HelpExampleRpcNamed(const std::string& methodname, const RPCArgList& args)
{
    ...
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"2.0\", \"id\": \"curltest\", "
           "\"method\": \"" + methodname + "\", \"params\": " + params.write() + "}' -H 'content-type: application/json' http://127.0.0.1/\n";
}
...
public:
    ...
    UniValue operator()(const DilithiumPKHash& keyID) const
    {
        ...
        obj.pushKV("isscript", false);
        obj.pushKV("iswitness", false);
        obj.pushKV("address", ::EncodeDestination(keyID));
        return obj;
    }
    ...
    UniValue operator()(const DilithiumScriptHash& scriptID) const
    {
        ...
        obj.pushKV("isscript", true);
        obj.pushKV("iswitness", false);
        obj.pushKV("address", ::EncodeDestination(scriptID));
        return obj;
    }
    ...
    UniValue operator()(const DilithiumWitnessV0KeyHash& id) const
    {
        ...
        obj.pushKV("witness_version", 0);
        obj.pushKV("witness_program", HexStr(id));
        obj.pushKV("address", ::EncodeDestination(id));
        return obj;
    }
