...
[[nodiscard]] static bool CreatePidFile(const ArgsManager& args)
{
    ...
    if (file) {
        ...
        return InitError(strprintf(_("Unable to create the PID file '%s': %s"), PathToString(GetPidFile(args)), SysErrorString(errno)));
    }
}
...
void SetupServerArgs(ArgsManager& argsman, bool can_listen_ipc)
{
    ...
    argsman.AddArg("-pqdemo", "Run Dilithium signature demo and exit (for testing)", ArgsManager::ALLOW_ANY, OptionsCategory::DEBUG_TEST);
#if HAVE_SYSTEM
    ...
    argsman.AddArg("-mineraddress=<addr>", "PQ address to receive coinbase rewards (required for getblocktemplate on mainnet/testnet)", ArgsManager::ALLOW_ANY, OptionsCategory::BLOCK_CREATION);
    argsman.AddArg("-minerdescriptor=<desc>", "Descriptor to derive coinbase script from (alternative to -mineraddress, must resolve to PQ address)", ArgsManager::ALLOW_ANY, OptionsCategory::BLOCK_CREATION);
    argsman.AddArg("-blockmintxfee=<amt>", strprintf("Set lowest fee rate (in %s/kvB) for transactions to be included in block creation. (default: %s)", CURRENCY_UNIT, FormatMoney(DEFAULT_BLOCK_MIN_TX_FEE)), ArgsManager::ALLOW_ANY, OptionsCategory::BLOCK_CREATION);
    argsman.AddArg("-blockversion=<n>", "Override block version to test forking scenarios", ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::BLOCK_CREATION);
    argsman.AddArg("-miningrequirespeers", strprintf("Require at least one peer connection to serve getblocktemplate. Set to 0 to allow solo mining / bootstrap the network. (default: %u)", 1), ArgsManager::ALLOW_ANY, OptionsCategory::BLOCK_CREATION);
...
bool AppInitParameterInteraction(const ArgsManager& args)
{
...

    {
        ...
        if (max_block_weight > LEGACY_MAX_BLOCK_WEIGHT) {
            return InitError(strprintf(_("Specified -blockmaxweight (%d) exceeds consensus maximum block weight (%d)"), max_block_weight, LEGACY_MAX_BLOCK_WEIGHT));
        }
    }

    {
        const auto block_reserved_weight = args.GetIntArg("-blockreservedweight", DEFAULT_BLOCK_RESERVED_WEIGHT);
        if (block_reserved_weight > LEGACY_MAX_BLOCK_WEIGHT) {
            return InitError(strprintf(_("Specified -blockreservedweight (%d) exceeds consensus maximum block weight (%d)"), block_reserved_weight, LEGACY_MAX_BLOCK_WEIGHT));
        }
...
static bool LockDirectory(const fs::path& dir, bool probeOnly)
{
    ...
    case util::LockResult::ErrorWrite:
        return InitError(strprintf(_("Cannot write to directory '%s'; check permissions."), PathToString(dir)));//fs
    case util::LockResult::ErrorLock:
        return InitError(strprintf(_("Cannot obtain a lock on directory %s. %s is probably already running."), PathToString(dir), CLIENT_NAME));//fs
...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if (args.IsArgSet("-datadir") && !args.GetPathArg("-datadir").is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if qbitx is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if qbitx is started while in a temporary directory.\n",
                  args.GetArg("-datadir", ""), PathToString(fs::current_path()));//fs
    }
...
// Sanity check: chain clients must never be null
for (size_t i = 0; i < node.chain_clients.size(); ++i) {
    if (!node.chain_clients[i]) {
        return InitError(Untranslated(strprintf("Null ChainClient in node.chain_clients at index %u", i)));
    }
}
LogPrintf("chain_clients size = %u\n", (unsigned)node.chain_clients.size());

// Create base chain client (node client). Must exist for full node RPC lifecycle.
if (node.chain_clients.empty()) {
    if (auto chain_client = node.init->makeChainClient()) {
        node.chain_clients.emplace_back(std::move(chain_client));
    } else {
        return InitError(Untranslated("Failed to create base ChainClient (makeChainClient returned null)"));
    }
}
...

    {
        ...
        if (args.IsArgSet("-asmap") && !args.IsArgNegated("-asmap")) {
            ...
            if (!fs::exists(asmap_path)) {
                InitError(strprintf(_("Could not find asmap file %s"), quoted(PathToString(asmap_path))));//fs
                return false;
            }
            asmap = DecodeAsmap(asmap_path);
            if (asmap.size() == 0) {
                InitError(strprintf(_("Could not parse asmap file %s"), quoted(PathToString(asmap_path))));//fs
                return false;
            }
    ...
    if (!CheckDiskSpace(args.GetDataDirNet())) {
        InitError(strprintf(_("Error: Disk space is low for %s"), quoted(PathToString(args.GetDataDirNet()))));//fs
        return false;
    }
    if (!CheckDiskSpace(args.GetBlocksDirPath())) {
        InitError(strprintf(_("Error: Disk space is low for %s"), quoted(PathToString(args.GetBlocksDirPath()))));//fs
        return false;
    }
    ...
    if (!do_reindex && !do_reindex_chainstate && chain_active_height <= 1) {
        ...
        if (!CheckDiskSpace(args.GetBlocksDirPath(), additional_bytes_needed)) {
            InitWarning(strprintf(_(
                ...
                quoted(PathToString(args.GetBlocksDirPath())),//fs
    ...
    for (const std::string& strFile : args.GetArgs("-loadblock")) {
        vImportFiles.push_back(PathFromString(strFile));//fs
    }
