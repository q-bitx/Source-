...
public:
    ...
    virtual std::unique_ptr<ChainClient> makeChainClient() { return nullptr; }
  //  virtual std::unique_ptr<ChainClient> makeChainClient() = 0;
};
...
namespace node {
    struct NodeContext;
    std::unique_ptr<interfaces::ChainClient> MakeChainClient(node::NodeContext& context);
}
