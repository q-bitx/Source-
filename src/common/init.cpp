// Copyright (c) 2025 The Q-BitX developers
...
std::optional<ConfigError> InitConfig(ArgsManager& args, SettingsAbortFn settings_abort_fn)
{
    try {
        ...
        // qbitx.conf file just ignores the other file.)
        ...
        // Show an error or warn/log if there is a qbitx.conf file in the
        ...
        if (fs::exists(base_config_path)) {
            if (orig_config_path.empty()) {
                LogInfo(
                    ...
                    ::quoted(::PathToString(base_path)),
                    ::quoted(BITCOIN_CONF_FILENAME));
                ...
                const std::string config_source = cli_config_path.empty()
                    ? strprintf("data directory %s", ::quoted(::PathToString(orig_datadir_path)))
                    : strprintf("command line argument %s", ::quoted("-conf=" + cli_config_path));
                std::string error = strprintf(
                    ...
                    ::quoted(::PathToString(base_path)),
                    ::quoted(BITCOIN_CONF_FILENAME),
                    ::quoted(::PathToString(orig_config_path)),
                    config_source);
