...
const char * const BITCOIN_CONF_FILENAME = "qbitx.conf";
...
fs::path ArgsManager::GetPathArg(std::string arg, const fs::path& default_value) const
{
   ...
   // fs::path result = fs::PathFromString(path_str).lexically_normal();
    fs::path result = PathFromString(path_str).lexically_normal();
...
fs::path ArgsManager::GetBlocksDirPath() const
{
   ...
   // path /= fs::PathFromString(BaseParams().DataDir());
    path /= PathFromString(BaseParams().DataDir());
    path /= "blocks";
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp, bool backup) const
{
    ...
    if (temp) {
            *filepath = fsbridge::AbsPathJoin(GetDataDirNet(), settings.string() + ".tmp");
        } else {
...
bool ArgsManager::WriteSettingsFile(std::vector<std::string>* errors, bool backup) const
{
    ...
    if (!RenameOver(path_tmp, path)) {
        SaveErrors({strprintf("Failed renaming settings file %s to %s\n", PathToString(path_tmp), PathToString(path))}, errors);
        return false;
    }
    return true;
}
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Q-BitX
    //   new: C:\Users\Username\AppData\Local\Q-BitX
    // macOS: ~/Library/Application Support/Q-BitX
    // Unix-like: ~/.qbitx
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Q-BitX";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Q-BitX";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/Q-BitX";
#else
    // Unix-like
    return pathRet / ".qbitx";
#endif
#endif
}
