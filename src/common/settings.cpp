...
bool ReadSettings(const fs::path& path, std::map<std::string, SettingsValue>& values, std::vector<std::string>& errors)
{
    ...
    file.open(path);
    if (!file.is_open()) {
      errors.emplace_back(strprintf("%s. Please check permissions.", ::PathToString(path)));
      return false;
    }
    ...
    if (file.fail()) {
        errors.emplace_back(strprintf("Failed reading settings file %s", ::PathToString(path)));
        return false;
    }
    file.close(); // Done with file descriptor. Release while copying data.

    if (!in.isObject()) {
        errors.emplace_back(strprintf("Found non-object value %s in settings file %s", in.write(), ::PathToString(path)));
        return false;
    }
    ...
    for (size_t i = 0; i < in_keys.size(); ++i) {
        ...
        if (!inserted.second) {
            errors.emplace_back(strprintf("Found duplicate key %s in settings file %s", in_keys[i], ::PathToString(path)));
            values.clear();
            break;
        }
    }
    ...
    std::vector<std::string>& errors)
{
    ...
    file.open(path);
    if (file.fail()) {
        errors.emplace_back(strprintf("Error: Unable to open settings file %s for writing", ::PathToString(path)));
        return false;
    }
