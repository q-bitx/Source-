...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    CAmount nSubsidy = (1250 * COIN) / 100;
...
bool CheckBlock(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // Q-BitX consensus rule: coinbase must pay to PQ address (except genesis block at height 0)
    // Format: OP_DUP OP_HASH160 <20-byte> OP_EQUALVERIFY OP_CHECKSIGDILITHIUM
    // Skip PQ enforcement for genesis block to allow existing networks to start up
    uint256 block_hash = block.GetHash();
    if (block_hash != consensusParams.hashGenesisBlock) {
        // For all blocks after genesis (height >= 1), enforce PQ coinbase
        if (block.vtx[0]->vout.empty() || !IsPQPayToPubKeyHash(block.vtx[0]->vout[0].scriptPubKey)) {
            return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cb-pq", "coinbase must pay to PQ address");
        }
    }
...
//        CScript expect = CScript() << nHeight;
	CScript expect = CScript() << int64_t(nHeight);
        ...
        try {
            ...
            if (!existed) {
                LogPrintf("[snapshot] snapshot chainstate dir being removed lacks %s file\n",
                          PathToString(node::SNAPSHOT_BLOCKHASH_FILENAME));
            }
        //} catch (const std::filesystem_error& e) {
          //  LogPrintf("[snapshot] failed to remove file %s: %s\n",
            //         PathToString(base_blockhash_path), fsbridge::get_filesystem_error_message(e));
        } catch (const std::filesystem::filesystem_error& e) {
        LogPrintf("[snapshot] failed to remove file %s: %s\n",
                  PathToString(base_blockhash_path),
                  fsbridge::get_filesystem_error_message(e));
	}
    }

    std::string path_str = PathToString(db_path);
...
util::Result<void> Chainstate::InvalidateCoinsDBOnDisk()
{
    ...
    std::string dbpath = PathToString(snapshot_datadir);
    std::string target = PathToString(invalid_path);
    ...
    try {
        ...
        auto src_str = PathToString(snapshot_datadir);
        auto dest_str = PathToString(invalid_path);
...
bool ChainstateManager::ValidatedSnapshotCleanup()
{
    ...
    LogPrintf("[snapshot] deleting background chainstate directory (now unnecessary) (%s)\n",
                  PathToString(ibd_chainstate_path));
...
tmp_old += "_todelete";
    ...
    auto rename_failed_abort = [this](
        ...
        LogError("[snapshot] Error renaming path (%s) -> (%s): %s\n",
                  PathToString(p_old), PathToString(p_new), err.what());
        GetNotifications().fatalError(strprintf(_(
            "Rename of '%s' -> '%s' failed. "
            "Cannot clean up the background chainstate leveldb directory."),
            PathToString(p_old), PathToString(p_new)));
    };
    ...
    LogPrintf("[snapshot] moving snapshot chainstate (%s) to "
              ...
              PathToString(snapshot_chainstate_path), PathToString(ibd_chainstate_path));
