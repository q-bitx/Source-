...
static bool ProcessInitCommands(ArgsManager& args)
{
    ...
    if (HelpRequested(args) || args.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME + std::string(" daemon version ") + FormatFullVersion() + "\n";
        strUsage += "The " + CLIENT_NAME + " daemon (qbitxd) is a headless program that connects to the Q-BitX network to validate and relay transactions and blocks, as well as relaying addresses.\n\n";
	if (args.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n";
		strUsage += "The " + CLIENT_NAME + " daemon (qbitxd) is a headless program that connects to the Q-BitX network to validate and relay transactions and blocks, as well as relaying addresses.\n";
            strUsage += "It provides the backbone of the Q-BitX network and its RPC, REST and ZMQ services can provide various transaction, block and address-related services.\n\n";
            strUsage += "There is an optional wallet component which provides transaction services.\n\n";
            strUsage += "It can be used in a headless environment or as part of a server setup.\n\n";
            strUsage += "Usage: qbitxd [options]\n\n";
...
void TestDilithium() {
    valtype pub, priv, msg, sig;
    msg = {'h', 'e', 'l', 'l', 'o'};

    if (!PQ_GenerateKeypair(pub, priv)) {
        std::cout << "KeyGen failed\n";
        return;
    }

    if (!PQ_Sign(sig, msg, priv)) {
        std::cout << "Sign failed\n";
        return;
    }

    if (!PQ_Verify(sig, msg, pub)) {
        std::cout << "Verify failed\n";
    } else {
        std::cout << "Success! Signature valid.\n";
    }
}
...
MAIN_FUNCTION
{
    ...
    util::SignalInterrupt signal_interrupt;
    node.shutdown_signal = &signal_interrupt;
    node.shutdown_request = [&signal_interrupt]() -> bool { return signal_interrupt(); };
    node.warnings = std::make_unique<node::Warnings>();
    ...
    if (!ParseArgs(node, argc, argv)) {
        std::cerr << "Error: Failed to parse command line arguments. See debug.log for details.\n";
        ECC_Stop();
        return EXIT_FAILURE;
    }

    if (ProcessInitCommands(gArgs)) {
        ECC_Stop();
        return EXIT_SUCCESS;
    }

    // Run Dilithium demo only if -pqdemo flag is set
    if (gArgs.GetBoolArg("-pqdemo", false)) {
        TestDilithium();
    }
...
//	if (auto chain = init->makeChain()) {
//        node.chain_clients.push_back(std::move(chain));
//	}
//node.chain = init->makeChain();
//if (!node.chain) {
//    ECC_Stop();
//    return EXIT_FAILURE;
//}
//if (auto chain_client = init->makeChainClient()) {
//    node.chain_clients.push_back(std::move(chain_client));
//}
