...
/**
 * Test Q-BitX consensus rule: coinbase must use PQ address format.
 * Format: OP_DUP OP_HASH160 <20-byte-hash> OP_EQUALVERIFY OP_CHECKSIGDILITHIUM
 */
BOOST_AUTO_TEST_CASE(coinbase_must_be_pq)
{
    LOCK(Assert(m_node.chainman)->GetMutex());
    
    // Create a valid PQ coinbase script: OP_DUP OP_HASH160 <20-byte> OP_EQUALVERIFY OP_CHECKSIGDILITHIUM
    std::vector<unsigned char> dummy_hash(20, 0x42);  // 20-byte dummy hash
    CScript valid_pq_coinbase = CScript() << OP_DUP << OP_HASH160 << dummy_hash << OP_EQUALVERIFY << OP_CHECKSIGDILITHIUM;
    
    // Create a block with valid PQ coinbase
    BlockAssembler::Options options;
    options.coinbase_output_script = valid_pq_coinbase;
    auto ptemplate = BlockAssembler{m_node.chainman->ActiveChainstate(), m_node.mempool.get(), options}.CreateNewBlock();
    CBlock valid_block = ptemplate->block;
    
    // Set up block header
    const CBlockIndex* tip = m_node.chainman->ActiveChain().Tip();
    valid_block.hashPrevBlock = tip->GetBlockHash();
    valid_block.nTime = tip->GetMedianTimePast() + 1;
    valid_block.nBits = GetNextWorkRequired(tip, &valid_block, Params().GetConsensus());
    valid_block.nNonce = 0;
    
    // Update coinbase scriptSig with height
    CMutableTransaction txCoinbase(*valid_block.vtx[0]);
    txCoinbase.vin[0].scriptSig = CScript() << static_cast<int64_t>(tip->nHeight + 1) << OP_0;
    valid_block.vtx[0] = MakeTransactionRef(std::move(txCoinbase));
    
    // Finalize block
    valid_block.hashMerkleRoot = BlockMerkleRoot(valid_block);
    m_node.chainman->GenerateCoinbaseCommitment(valid_block, tip);
    valid_block.hashMerkleRoot = BlockMerkleRoot(valid_block);
    
    // Solve PoW
    while (!CheckProofOfWork(valid_block.GetHash(), valid_block.nBits, Params().GetConsensus())) {
        ++(valid_block.nNonce);
    }
    
    // Test that valid PQ coinbase passes CheckBlock
    BlockValidationState state;
    BOOST_CHECK(CheckBlock(valid_block, state, Params().GetConsensus(), true, true));
    BOOST_CHECK(state.IsValid());
    
    // Create an invalid coinbase script (legacy P2PKH with OP_CHECKSIG)
    std::vector<unsigned char> legacy_hash(20, 0x43);
    CScript invalid_legacy_coinbase = CScript() << OP_DUP << OP_HASH160 << legacy_hash << OP_EQUALVERIFY << OP_CHECKSIG;
    
    // Create a block with invalid legacy coinbase
    BlockAssembler::Options invalid_options;
    invalid_options.coinbase_output_script = invalid_legacy_coinbase;
    auto invalid_template = BlockAssembler{m_node.chainman->ActiveChainstate(), m_node.mempool.get(), invalid_options}.CreateNewBlock();
    CBlock invalid_block = invalid_template->block;
    
    // Set up block header
    invalid_block.hashPrevBlock = tip->GetBlockHash();
    invalid_block.nTime = tip->GetMedianTimePast() + 1;
    invalid_block.nBits = GetNextWorkRequired(tip, &invalid_block, Params().GetConsensus());
    invalid_block.nNonce = 0;
    
    // Update coinbase scriptSig with height
    CMutableTransaction txCoinbaseInvalid(*invalid_block.vtx[0]);
    txCoinbaseInvalid.vin[0].scriptSig = CScript() << static_cast<int64_t>(tip->nHeight + 1) << OP_0;
    invalid_block.vtx[0] = MakeTransactionRef(std::move(txCoinbaseInvalid));
    
    // Finalize block
    invalid_block.hashMerkleRoot = BlockMerkleRoot(invalid_block);
    m_node.chainman->GenerateCoinbaseCommitment(invalid_block, tip);
    invalid_block.hashMerkleRoot = BlockMerkleRoot(invalid_block);
    
    // Solve PoW
    while (!CheckProofOfWork(invalid_block.GetHash(), invalid_block.nBits, Params().GetConsensus())) {
        ++(invalid_block.nNonce);
    }
    
    // Test that invalid legacy coinbase fails CheckBlock with "bad-cb-pq"
    BlockValidationState invalid_state;
    BOOST_CHECK(!CheckBlock(invalid_block, invalid_state, Params().GetConsensus(), true, true));
    BOOST_CHECK(!invalid_state.IsValid());
    BOOST_CHECK_EQUAL(invalid_state.GetRejectReason(), "bad-cb-pq");
    BOOST_CHECK(invalid_state.GetDebugMessage().find("coinbase must pay to PQ address") != std::string::npos);
}
