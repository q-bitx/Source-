...
BOOST_AUTO_TEST_CASE(eda_uses_pure_block_times)
{
    const auto chainParams = CreateChainParams(*m_node.args, ChainType::MAIN);
    const auto& consensus = chainParams->GetConsensus();

    // This test is only meaningful if EDA is enabled in params.
    if (!consensus.fPowEnableEmergencyDifficultyDrop) return;

    // Set up a last block that is NOT at a retarget boundary, with a known nBits.
    CBlockIndex last;
    last.nHeight = 100; // (100+1) % 2016 != 0
    last.nTime = 1'000'000;
    last.nBits = 0x1c387f6f; // arbitrary non-powLimit difficulty from existing test vectors above

    // Candidate header with a time gap > emergency window should trigger EDA.
    CBlockHeader candidate;
    candidate.nTime = last.GetBlockTime() + consensus.nPowEmergencyWindow + 1;

    const unsigned int new_bits = GetNextWorkRequired(&last, &candidate, consensus);

    // Expected: new_target = min(old_target * 2, powLimit), compared as arith_uint256.
    arith_uint256 old_target;
    old_target.SetCompact(last.nBits);
    arith_uint256 expected_target = old_target;
    expected_target <<= 1;
    const arith_uint256 pow_limit = UintToArith256(consensus.powLimit);
    if (expected_target > pow_limit) expected_target = pow_limit;

    arith_uint256 new_target;
    new_target.SetCompact(new_bits);
    BOOST_CHECK(new_target == expected_target);

    // Consensus should permit this exact transition.
    BOOST_CHECK(PermittedDifficultyTransition(consensus, last.nHeight + 1, last.nBits, new_bits));
}
