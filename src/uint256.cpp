...
bool uint256::SetHex(std::string_view str) {
    if (!IsHex(str)) return false;
    std::vector<uint8_t> v = ParseHex(str);
    if (v.size() != sizeof(uint256)) return false;
    std::reverse(v.begin(), v.end());
    std::memcpy(this->begin(), v.data(), sizeof(uint256));
    return true;
}

bool uint256::SetHex(const char* str) {
    return SetHex(std::string_view(str));
}
