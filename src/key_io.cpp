// Copyright (c) 2014-2021 The Q-BitX developers
...
public:
    ...
    std::string operator()(const PKHash& id) const
    {
        ...
        // Append hash bytes explicitly (unsigned char, not std::byte)
...
CTxDestination DecodeDestination(const std::string& str, const CChainParams& params, std::string& error_str, std::vector<int>* error_locations)
{
    ...
    if (!is_bech32 && !is_dilithium_bech32 && DecodeBase58Check(str, data, 21)) {
        // base58-encoded Q-BitX addresses.
...
std::string EncodeSecret(const CKey& key)
{
    ...
    // Append key bytes explicitly (unsigned char, not std::byte)
...
CDilithiumKey DecodeDilithiumSecret(const std::string& str)
{
    CDilithiumKey key;
    std::vector<unsigned char> data;
    // Dilithium keys are much larger than ECDSA keys (2528 bytes)
    const size_t max_dilithium_wif_size = CDilithiumKey::SIZE + 10; // prefix + checksum overhead
    if (DecodeBase58Check(str, data, max_dilithium_wif_size)) {
        const std::vector<unsigned char>& privkey_prefix = Params().Base58Prefix(CChainParams::SECRET_KEY);
        if (data.size() == CDilithiumKey::SIZE + privkey_prefix.size() &&
            std::equal(privkey_prefix.begin(), privkey_prefix.end(), data.begin())) {
            std::copy(
                data.begin() + privkey_prefix.size(),
                data.begin() + privkey_prefix.size() + CDilithiumKey::SIZE,
                key.data()
            );
        }
    }
...
std::string EncodeDilithiumSecret(const CDilithiumKey& key)
{
    ...
    std::vector<unsigned char> data = Params().Base58Prefix(CChainParams::SECRET_KEY);
    // Append key bytes explicitly (unsigned char, not std::byte)
    for (const unsigned char* it = key.begin(); it != key.end(); ++it) {
        data.push_back(*it);
    }
    std::string ret = EncodeBase58Check(data);
    memory_cleanse(data.data(), data.size());
