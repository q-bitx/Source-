...
static std::optional<int> ReadPragmaInteger(sqlite3* db, const std::string& key, const std::string& description, bilingual_str& error)
{
    ...
    if (ret != SQLITE_OK) {
        ...
        LogPrintf("[wallet-sqlite] rc=%d xrc=%d msg=%s sql=%s\n", ret, xrc, sqlite3_errmsg(db), stmt_text);
    ...
    int finalize_ret = sqlite3_finalize(pragma_read_stmt);
    if (finalize_ret != SQLITE_OK) {
        int xrc = sqlite3_extended_errcode(db);
        LogPrintf("[wallet-sqlite] rc=%d xrc=%d msg=%s sql=%s\n", finalize_ret, xrc, sqlite3_errmsg(db), stmt_text);
    }
    return result;
}
...
static void SetPragma(sqlite3* db, const std::string& key, const std::string& value, const std::string& err_msg)
{
    ...
    if (ret != SQLITE_OK) {
        int xrc = sqlite3_extended_errcode(db);
        LogPrintf("[wallet-sqlite] rc=%d xrc=%d msg=%s sql=%s\n", ret, xrc, sqlite3_errmsg(db), stmt_text);
        std::string error_str = strprintf("SQLiteDatabase: %s: %s\n", err_msg, sqlite3_errstr(ret));
...
SQLiteDatabase::SQLiteDatabase(const fs::path& dir_path, const fs::path& file_path, const DatabaseOptions& options, bool mock)
    : WalletDatabase(), m_mock(mock), m_dir_path(PathToString(dir_path)), m_file_path(PathToString(file_path)), m_write_semaphore(1), m_use_unsafe_sync(options.use_unsafe_sync)
{
    {
        LOCK(g_sqlite_mutex);
        ...
        if (++g_sqlite_count == 1) {
            ...
            if (ret != SQLITE_OK) {
                std::string err_msg = strprintf("SQLiteDatabase: Failed to setup error log: %s\n", sqlite3_errstr(ret));
                LogPrintf("[walletdb] SQLite error: %s", err_msg);
                throw std::runtime_error(err_msg);
            }
            // Force serialized threading mode
            ret = sqlite3_config(SQLITE_CONFIG_SERIALIZED);
            if (ret != SQLITE_OK) {
                std::string err_msg = strprintf("SQLiteDatabase: Failed to configure serialized threading mode: %s\n", sqlite3_errstr(ret));
                LogPrintf("[walletdb] SQLite error: %s", err_msg);
                throw std::runtime_error(err_msg);
            }
        }
        ...
        if (ret != SQLITE_OK) {
            std::string err_msg = strprintf("SQLiteDatabase: Failed to initialize SQLite: %s\n", sqlite3_errstr(ret));
            LogPrintf("[walletdb] SQLite error: %s", err_msg);
            throw std::runtime_error(err_msg);
        }
    }
...
void SQLiteBatch::SetupSQLStatements()
{
    ...
    for (const auto& [stmt_prepared, stmt_text] : statements) {
        if (*stmt_prepared == nullptr) {
            ...
            if (res != SQLITE_OK) {
                int xrc = sqlite3_extended_errcode(m_database.m_db);
                LogPrintf("[wallet-sqlite] rc=%d xrc=%d msg=%s sql=%s\n", res, xrc, sqlite3_errmsg(m_database.m_db), stmt_text);
...
bool SQLiteDatabase::Verify(bilingual_str& error)
{
    ...
    if (!read_result.has_value()) {
        LogPrintf("[walletdb] SQLite error: %s\n", error.original);
        return false;
    }
    ...
    int ret = sqlite3_prepare_v2(m_db, integrity_sql, -1, &stmt, nullptr);
    if (ret != SQLITE_OK) {
        int xrc = sqlite3_extended_errcode(m_db);
        LogPrintf("[wallet-sqlite] rc=%d xrc=%d msg=%s sql=%s\n", ret, xrc, sqlite3_errmsg(m_db), integrity_sql);
    ...
    if (finalize_ret != SQLITE_OK) {
        int xrc = sqlite3_extended_errcode(m_db);
        LogPrintf("[wallet-sqlite] rc=%d xrc=%d msg=%s sql=%s\n", finalize_ret, xrc, sqlite3_errmsg(m_db), integrity_sql);
    }
    if (!error.empty()) {
        LogPrintf("[walletdb] SQLite error: %s\n", error.original);
    }
...
void SQLiteDatabase::Open()
{
    ...
    if (m_db == nullptr) {
        if (!m_mock) {
            TryCreateDirectories(PathFromString(m_dir_path));
        }
        int ret = sqlite3_open_v2(m_file_path.c_str(), &m_db, flags, nullptr);
        if (ret != SQLITE_OK) {
            int xrc = sqlite3_extended_errcode(m_db);
            LogPrintf("[wallet-sqlite] rc=%d xrc=%d msg=%s sql=sqlite3_open_v2\n", ret, xrc, sqlite3_errmsg(m_db));
            std::string err_msg = strprintf("SQLiteDatabase: Failed to open database: %s\n", sqlite3_errstr(ret));
            throw std::runtime_error(err_msg);
        }
        ret = sqlite3_extended_result_codes(m_db, 1);
        if (ret != SQLITE_OK) {
            int xrc = sqlite3_extended_errcode(m_db);
            LogPrintf("[wallet-sqlite] rc=%d xrc=%d msg=%s sql=sqlite3_extended_result_codes\n", ret, xrc, sqlite3_errmsg(m_db));
            std::string err_msg = strprintf("SQLiteDatabase: Failed to enable extended result codes: %s\n", sqlite3_errstr(ret));
            throw std::runtime_error(err_msg);
        }
    ...
    if (sqlite3_db_readonly(m_db, "main") != 0) {
        std::string err_msg = "SQLiteDatabase: Database opened in readonly mode but read-write permissions are needed";
        LogPrintf("[walletdb] SQLite error: %s\n", err_msg);
        throw std::runtime_error(err_msg);
    }
...
if (ret != SQLITE_OK) {
    ...
    LogPrintf("[wallet-sqlite] rc=%d xrc=%d msg=%s sql=%s\n", ret, xrc, sqlite3_errmsg(m_db), sql);
    std::string err_msg = std::string("SQLiteDatabase: Unable to obtain an exclusive lock on the database, is it being used by another instance of ") +
        CLIENT_NAME + "?\n";
    throw std::runtime_error(err_msg);
}
...
if (ret != SQLITE_OK) {
    ...
    LogPrintf("[wallet-sqlite] rc=%d xrc=%d msg=%s sql=%s\n", ret, xrc, sqlite3_errmsg(m_db), sql);
    std::string err_msg = strprintf("SQLiteDatabase: Unable to end exclusive lock transaction: %s\n", sqlite3_errstr(ret));
    throw std::runtime_error(err_msg);
}
    ...
    const char* check_table_sql = "SELECT name FROM sqlite_master WHERE type='table' AND name='main'";
    ret = sqlite3_prepare_v2(m_db, check_table_sql, -1, &check_main_stmt, nullptr);
    if (ret != SQLITE_OK) {
        int xrc = sqlite3_extended_errcode(m_db);
        LogPrintf("[wallet-sqlite] rc=%d xrc=%d msg=%s sql=%s\n", ret, xrc, sqlite3_errmsg(m_db), check_table_sql);
        std::string err_msg = strprintf("SQLiteDatabase: Failed to prepare statement to check table existence: %s\n", sqlite3_errstr(ret));
        throw std::runtime_error(err_msg);
    }
    ret = sqlite3_step(check_main_stmt);
    int finalize_ret = sqlite3_finalize(check_main_stmt);
    if (finalize_ret != SQLITE_OK) {
        int xrc = sqlite3_extended_errcode(m_db);
        LogPrintf("[wallet-sqlite] rc=%d xrc=%d msg=%s sql=%s\n", finalize_ret, xrc, sqlite3_errmsg(m_db), check_table_sql);
        std::string err_msg = strprintf("SQLiteDatabase: Failed to finalize statement checking table existence: %s\n", sqlite3_errstr(finalize_ret));
        throw std::runtime_error(err_msg);
    }
    bool table_exists;
    if (ret == SQLITE_DONE) {
        ...
        int xrc = sqlite3_extended_errcode(m_db);
        LogPrintf("[wallet-sqlite] rc=%d xrc=%d msg=%s sql=%s\n", ret, xrc, sqlite3_errmsg(m_db), check_table_sql);
        std::string err_msg = strprintf("SQLiteDatabase: Failed to execute statement to check table existence: %s\n", sqlite3_errstr(ret));
        throw std::runtime_error(err_msg);
    }

    // Do the db setup things because the table doesn't exist only when we are creating a new wallet
    if (!table_exists) {
        const char* create_table_sql = "CREATE TABLE main(key BLOB PRIMARY KEY NOT NULL, value BLOB NOT NULL)";
        ret = sqlite3_exec(m_db, create_table_sql, nullptr, nullptr, nullptr);
        if (ret != SQLITE_OK) {
            int xrc = sqlite3_extended_errcode(m_db);
            LogPrintf("[wallet-sqlite] rc=%d xrc=%d msg=%s sql=%s\n", ret, xrc, sqlite3_errmsg(m_db), create_table_sql);
            std::string err_msg = strprintf("SQLiteDatabase: Failed to create new database: %s\n", sqlite3_errstr(ret));
            throw std::runtime_error(err_msg);
        }
...
bool SQLiteDatabase::Backup(const std::string& dest) const
{
    ...
    if (res != SQLITE_OK) {
        int xrc = sqlite3_extended_errcode(db_copy);
        LogPrintf("[wallet-sqlite] rc=%d xrc=%d msg=%s sql=sqlite3_open\n", res, xrc, sqlite3_errmsg(db_copy));
        sqlite3_close(db_copy);
        return false;
    }
    sqlite3_backup* backup = sqlite3_backup_init(db_copy, "main", m_db, "main");
    if (!backup) {
        int xrc_src = sqlite3_extended_errcode(m_db);
        int xrc_dest = sqlite3_extended_errcode(db_copy);
        LogPrintf("[wallet-sqlite] rc=N/A xrc_src=%d xrc_dest=%d msg_src=%s msg_dest=%s sql=sqlite3_backup_init\n",
                 xrc_src, xrc_dest, sqlite3_errmsg(m_db), sqlite3_errmsg(db_copy));
        sqlite3_close(db_copy);
        return false;
    }
    ...
    if (res != SQLITE_DONE) {
        int xrc_src = sqlite3_extended_errcode(m_db);
        int xrc_dest = sqlite3_extended_errcode(db_copy);
        LogPrintf("[wallet-sqlite] rc=%d xrc_src=%d xrc_dest=%d msg_src=%s msg_dest=%s sql=sqlite3_backup_step\n",
                 res, xrc_src, xrc_dest, sqlite3_errmsg(m_db), sqlite3_errmsg(db_copy));
    ...
    if (res != SQLITE_OK) {
        int xrc_src = sqlite3_extended_errcode(m_db);
        int xrc_dest = sqlite3_extended_errcode(db_copy);
        LogPrintf("[wallet-sqlite] rc=%d xrc_src=%d xrc_dest=%d msg_src=%s msg_dest=%s sql=sqlite3_backup_finish\n",
                 res, xrc_src, xrc_dest, sqlite3_errmsg(m_db), sqlite3_errmsg(db_copy));
    }
    sqlite3_close(db_copy);
...
void SQLiteDatabase::Close()
{
    ...
    if (res != SQLITE_OK) {
        int xrc = sqlite3_extended_errcode(m_db);
        LogPrintf("[wallet-sqlite] rc=%d xrc=%d msg=%s sql=sqlite3_close\n", res, xrc, sqlite3_errmsg(m_db));
...
void SQLiteBatch::Close()
{
    ...
    for (const auto& [stmt_prepared, stmt_description] : statements) {
        ...
        if (res != SQLITE_OK) {
            int xrc = sqlite3_extended_errcode(m_database.m_db);
            const char* sql = stmt_description;
            LogPrintf("[wallet-sqlite] rc=%d xrc=%d msg=%s sql=sqlite3_finalize(%s)\n", res, xrc, sqlite3_errmsg(m_database.m_db), sql);
        }
...
bool SQLiteBatch::WriteKey(DataStream&& key, DataStream&& value, bool overwrite)
{
    ...
    if (res != SQLITE_DONE) {
        int xrc = sqlite3_extended_errcode(m_database.m_db);
        const char* sql = overwrite ? "INSERT or REPLACE into main values(?, ?)" : "INSERT INTO main VALUES(?, ?)";
        LogPrintf("[wallet-sqlite] rc=%d xrc=%d msg=%s sql=%s\n", res, xrc, sqlite3_errmsg(m_database.m_db), sql);
    }
...
bool SQLiteBatch::ExecStatement(sqlite3_stmt* stmt, std::span<const std::byte> blob)
{
    ...
    if (res != SQLITE_DONE) {
        int xrc = sqlite3_extended_errcode(m_database.m_db);
        const char* sql = (stmt == m_delete_stmt) ? "DELETE FROM main WHERE key = ?" : "DELETE FROM main WHERE instr(key, ?) = 1";
        LogPrintf("[wallet-sqlite] rc=%d xrc=%d msg=%s sql=%s\n", res, xrc, sqlite3_errmsg(m_database.m_db), sql);
    }
...
DatabaseCursor::Status SQLiteCursor::Next(DataStream& key, DataStream& value)
{
    ...
    int xrc = db ? sqlite3_extended_errcode(db) : 0;
