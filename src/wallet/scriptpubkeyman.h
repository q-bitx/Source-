...
public:
    ...
    virtual bool SignTransaction(CMutableTransaction& tx, const std::map<COutPoint, Coin>& coins, int sighash, std::map<int, bilingual_str>& input_errors, bool pq_witness_program_templates = false) const { return false; }
    /** Sign a message with the given script */
    virtual SigningResult SignMessage(const std::string& message, const PKHash& pkhash, std::string& str_sig) const { return SigningResult::SIGNING_FAILED; };
    /** Adds script and derivation path information to a PSBT, and optionally signs it. */
    virtual std::optional<common::PSBTError> FillPSBT(PartiallySignedTransaction& psbt, const PrecomputedTransactionData& txdata, int sighash_type = SIGHASH_DEFAULT, bool sign = true, bool bip32derivs = false, int* n_signed = nullptr, bool finalize = true, bool pq_witness_program_templates = false) const { return common::PSBTError::UNSUPPORTED; }
...
public:
    ...
    bool SignTransaction(CMutableTransaction& tx, const std::map<COutPoint, Coin>& coins, int sighash, std::map<int, bilingual_str>& input_errors, bool pq_witness_program_templates = false) const override;
    SigningResult SignMessage(const std::string& message, const PKHash& pkhash, std::string& str_sig) const override;
    std::optional<common::PSBTError> FillPSBT(PartiallySignedTransaction& psbt, const PrecomputedTransactionData& txdata, int sighash_type = SIGHASH_DEFAULT, bool sign = true, bool bip32derivs = false, int* n_signed = nullptr, bool finalize = true, bool pq_witness_program_templates = false) const override;
...
private:
    ...
    using DilithiumKeyMap = std::map<CKeyID, std::vector<unsigned char>>; // Map from CKeyID to Dilithium private key bytes
    using CryptedDilithiumKeyMap = std::map<CKeyID, std::pair<std::vector<unsigned char>, std::vector<unsigned char>>>; // Map from CKeyID to (pubkey_bytes, crypted_privkey_bytes)
    ...
    bool AddDilithiumKeyWithDB(WalletBatch& batch, const CKeyID& keyid, const std::vector<unsigned char>& vchPrivKey, const std::vector<unsigned char>& vchPubKey) EXCLUSIVE_LOCKS_REQUIRED(cs_desc_man);
    bool LoadDilithiumKey(const CKeyID& keyid, const std::vector<unsigned char>& vchPrivKey, const std::vector<unsigned char>& vchPubKey);
    bool GetDilithiumKey_Locked(const CKeyID& keyid, std::vector<unsigned char>& vchPrivKey) const EXCLUSIVE_LOCKS_REQUIRED(cs_desc_man);
    bool HaveDilithiumKey_Locked(const CKeyID& keyid) const EXCLUSIVE_LOCKS_REQUIRED(cs_desc_man);
...
public:
    ...
    // Dilithium key management
    bool AddDilithiumKeyPubKey(const CKeyID& keyid, const std::vector<unsigned char>& vchPrivKey, const std::vector<unsigned char>& vchPubKey);
    bool LoadCryptedDilithiumKey(const CKeyID& keyid, const std::vector<unsigned char>& vchPubKey, const std::vector<unsigned char>& vchCryptedSecret);
    bool LoadDilithiumKeyFromDB(const CKeyID& keyid, const std::vector<unsigned char>& vchPrivKey, const std::vector<unsigned char>& vchPubKey);
    bool HaveDilithiumKey(const CKeyID& keyid) const;
    bool GetDilithiumKey(const CKeyID& keyid, std::vector<unsigned char>& vchPrivKey) const;
    bool GetDilithiumKeys(const CKeyID& keyid, std::vector<unsigned char>& vchPrivKey, std::vector<unsigned char>& vchPubKey) const;
