...
{
    LOCK(cs_db);
    auto inserted = g_dbenvs.emplace(PathToString(env_directory), std::weak_ptr<BerkeleyEnvironment>());
    if (inserted.second) {
...
void BerkeleyEnvironment::Close()
{
    ...
    UnlockDirectory(PathFromString(strPath), ".walletlock");
}
...
BerkeleyEnvironment::BerkeleyEnvironment(const fs::path& dir_path, bool use_shared_memory) : strPath(PathToString(dir_path)), m_use_shared_memory(use_shared_memory)
{
    Reset();
}
...
bool BerkeleyEnvironment::Open(bilingual_str& err)
{
    ...
    fs::path pathIn = PathFromString(strPath);
    ...
    if (util::LockDirectory(pathIn, ".walletlock") != util::LockResult::Success) {
      ...
      //  err = strprintf(_("Error initializing wallet database environment %s!", quoted(PathToString(Directory())));
	err = strprintf(
        _("Error initializing wallet database environment %s!"),
        quoted(PathToString(pathIn))
    );
        return false;
    }
    ...
    LogPrintf("BerkeleyEnvironment::Open: LogDir=%s ErrorFile=%s\n", PathToString(pathLogDir), PathToString(pathErrorFile));
    ...
    dbenv->set_lg_dir(PathToString(pathLogDir).c_str());
...
bool BerkeleyDatabase::Verify(bilingual_str& errorStr)
{
    ...
    if (fs::exists(file_path))
    {
        ...
        const std::string strFile = PathToString(m_filename);
        int result = db.verify(strFile.c_str(), nullptr, nullptr, 0);
        if (result != 0) {
            errorStr = strprintf(_("%s corrupt. Try using the wallet tool bitcoin-wallet to salvage or restoring a backup."), quoted(PathToString(file_path)));
            return false;
        }
    }
...
BerkeleyDatabase::~BerkeleyDatabase()
{
    if (env) {
        ...
        assert(erased == 1);
        env->m_fileids.erase(PathToString(m_filename));
    }
}
...
BerkeleyBatch::BerkeleyBatch(BerkeleyDatabase& database, const bool read_only, bool fFlushOnCloseIn) : m_database(database)
{
    ...
    strFile = PathToString(database.m_filename);
}
...
void BerkeleyEnvironment::Flush(bool fShutdown)
{
        ...
        return;
    {
        ...
        for (auto& db_it : m_databases) {
            ...
            const std::string strFile = PathToString(filename);
        ...
        if (fShutdown) {
            ...
            if (no_dbs_accessed) {
                ...
                Close();
                if (!fMockDb) {
                    fs::remove_all(PathFromString(strPath) / "database");
                }
            }
        }
    }
    ...
    {
        {
            ...
            if (m_refcount <= 0)
            {
                ...
                fs::path pathDest(PathFromString(strDest));
                if (fs::is_directory(pathDest))
                    pathDest /= m_filename;

                try {
                    if (fs::exists(pathDest) && fs::equivalent(pathSrc, pathDest)) {
                        LogPrintf("cannot backup to wallet source file %s\n", PathToString(pathDest));
                        return false;
                    }

                    fs::copy_file(pathSrc, pathDest, fs::copy_options::overwrite_existing);
                    LogPrintf("copied %s to %s\n", strFile, PathToString(pathDest));
                    return true;
                } catch (const fs::filesystem_error& e) {
                    LogPrintf("error copying %s to %s - %s\n", strFile, PathToString(pathDest), fsbridge::get_filesystem_error_message(e));
                    return false;
                }
            }
        }
...
std::unique_ptr<BerkeleyDatabase> MakeBerkeleyDatabase(const fs::path& path, const DatabaseOptions& options, DatabaseStatus& status, bilingual_str& error)
{
    ...
    std::unique_ptr<BerkeleyDatabase> db;
    {
        ...
        if (env->m_databases.count(data_filename)) {
            error = Untranslated(strprintf("Refusing to load database. Data file '%s' is already loaded.", PathToString(env->Directory() / data_filename)));
