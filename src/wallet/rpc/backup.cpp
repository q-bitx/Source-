        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (fs::exists(filepath)) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, PathToString(filepath) + " already exists. If you are sure this is what you want, move it out of the way first");
    }
    ...
    reply.pushKV("filename", PathToString(filepath));
...
RPCHelpMan backupwallet()
{
    return RPCHelpMan{"backupwallet",
                "\nCopies the entire wallet directory to the specified destination directory.\n"
                "Safe for descriptor wallets. You can back up to a USB drive and restore on another computer by copying the folder.\n",
                {
                    {"destination", RPCArg::Type::STR, RPCArg::Optional::NO, "The destination directory"},
                },
                RPCResult{
                    RPCResult::Type::OBJ, "", "",
                    {
                        {RPCResult::Type::STR, "wallet", "The wallet name"},
                        {RPCResult::Type::STR, "backup_path", "The full path to the backup directory"},
                        {RPCResult::Type::STR, "method", "The backup method used"},
                    }
                },
                RPCExamples{
                    HelpExampleCli("backupwallet", "\"/media/usb\"")
            + HelpExampleRpc("backupwallet", "\"/media/usb\"")
                },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    const std::shared_ptr<const CWallet> pwallet = GetWalletForJSONRPCRequest(request);
    ...
    fs::path dest_dir = fs::u8path(request.params[0].get_str());
    dest_dir = fs::absolute(dest_dir);

    // Validate destination is a directory
    if (fs::exists(dest_dir) && !fs::is_directory(dest_dir)) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Destination must be a directory");
    }

    // Create backup directory: <destination>/<wallet_name>/
    fs::path backup_path = dest_dir / fs::u8path(pwallet->GetName());

    // Check if backup directory already exists and is not empty
    if (fs::exists(backup_path)) {
        bool has_files = false;
        for (auto it = fs::directory_iterator(backup_path); it != fs::directory_iterator(); ++it) {
            has_files = true;
            break;
        }
        if (has_files) {
            throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("Backup directory '%s' already exists and is not empty", PathToString(backup_path)));
        }
    }

    // Create destination directory if it doesn't exist
    if (!TryCreateDirectories(dest_dir)) {
        throw JSONRPCError(RPC_WALLET_ERROR, strprintf("Failed to create destination directory '%s'", PathToString(dest_dir)));
    }

    // Create backup subdirectory
    if (!TryCreateDirectories(backup_path)) {
        throw JSONRPCError(RPC_WALLET_ERROR, strprintf("Failed to create backup directory '%s'", PathToString(backup_path)));
    }

    // Create backup file path: backup_dir / "wallet.dat"
    fs::path backup_file = backup_path / "wallet.dat";

    // Use SQLite backup API to create backup
    if (!pwallet->BackupWallet(PathToString(backup_file))) {
        throw JSONRPCError(RPC_WALLET_ERROR, "Error: Unable to make a backup of your wallet");
    }

    UniValue result(UniValue::VOBJ);
    result.pushKV("wallet", pwallet->GetName());
    result.pushKV("backup_path", PathToString(backup_path));
    result.pushKV("method", "sqlite-backup-api");
...
RPCHelpMan restorewallet()
{
    return RPCHelpMan{
        "restorewallet",
        "\nRestores a wallet from backup directory.\n"
        "\nCreates a new wallet with the specified name by copying the wallet directory from the source location.\n"
        "You can back up to a USB drive and restore on another computer by copying the folder.\n",
        {
            {"wallet_name", RPCArg::Type::STR, RPCArg::Optional::NO, "The name that will be applied to the restored wallet"},
            {"source_dir", RPCArg::Type::STR, RPCArg::Optional::NO, "The source directory containing the wallet backup"},
            {"load_on_startup", RPCArg::Type::BOOL, RPCArg::Optional::OMITTED, "Save wallet name to persistent settings and load on startup. True to add wallet to startup list, false to remove, null to leave unchanged."},
        },
        RPCResult{
            RPCResult::Type::OBJ, "", "",
            {
                {RPCResult::Type::STR, "wallet", "The wallet name if restored successfully."},
                {RPCResult::Type::STR, "restored_from", "The source directory path"},
                {RPCResult::Type::BOOL, "loaded", "Whether the wallet was loaded successfully"},
            }
        },
        RPCExamples{
            HelpExampleCli("restorewallet", "\"mywallet\" \"/media/usb/mywallet\"")
            + HelpExampleRpc("restorewallet", "\"mywallet\" \"/media/usb/mywallet\"")
            + HelpExampleCliNamed("restorewallet", {{"wallet_name", "mywallet"}, {"source_dir", "/media/usb/mywallet"}, {"load_on_startup", true}})
            + HelpExampleRpcNamed("restorewallet", {{"wallet_name", "mywallet"}, {"source_dir", "/media/usb/mywallet"}, {"load_on_startup", true}})
        },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    fs::path source_dir = fs::u8path(request.params[1].get_str());
    ...
    if (!fs::exists(source_dir)) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("Source directory '%s' does not exist", PathToString(source_dir)));
    }

    if (!fs::is_directory(source_dir)) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("Source path '%s' is not a directory", PathToString(source_dir)));
    }
    ...
    if (fs::exists(wallet_path)) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("Wallet '%s' already exists", wallet_name));
    }

    // Create wallet directory
    if (!TryCreateDirectories(wallet_path)) {
        throw JSONRPCError(RPC_WALLET_ERROR, strprintf("Failed to create wallet directory '%s'", PathToString(wallet_path)));
    }

    // Copy ONLY wallet.dat from source directory (do NOT copy journal/wal/shm files)
    fs::path source_wallet_file = source_dir / "wallet.dat";
    fs::path dest_wallet_file = wallet_path / "wallet.dat";

    // Validate that wallet.dat exists in source directory
    if (!fs::exists(source_wallet_file)) {
        fs::remove_all(wallet_path);
        throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("Source directory '%s' does not contain wallet.dat", PathToString(source_dir)));
    }

    if (!fs::is_regular_file(source_wallet_file)) {
        fs::remove_all(wallet_path);
        throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("Source path '%s' is not a regular file", PathToString(source_wallet_file)));
    }

    // Copy wallet.dat file
    std::error_code ec;
    fs::copy_file(source_wallet_file, dest_wallet_file, fs::copy_options::none, ec);
    if (ec) {
        fs::remove_all(wallet_path);
        throw JSONRPCError(RPC_WALLET_ERROR, strprintf("Error copying wallet.dat from '%s' to '%s': %s", PathToString(source_wallet_file), PathToString(dest_wallet_file), ec.message()));
    }

    // Load the wallet
    DatabaseOptions options;
    options.require_existing = true;
    const std::shared_ptr<CWallet> wallet = LoadWallet(context, wallet_name, load_on_start, options, status, error, warnings);
    ...
    obj.pushKV("wallet", wallet->GetName());
    obj.pushKV("restored_from", PathToString(source_dir));
    obj.pushKV("loaded", true);
