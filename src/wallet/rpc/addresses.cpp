...
RPCHelpMan getnewaddress()
{
    return RPCHelpMan{"getnewaddress",
                ...
                RPCExamples{
                    "\nGet a new PQ (Dilithium) address (default)\n"
                    + HelpExampleCli("getnewaddress", "") +
                    "\nGet a new PQ address with a label\n"
                    + HelpExampleCli("getnewaddress", "\"mylabel\"") +
                    "\nGet a new PQ address explicitly\n"
                    + HelpExampleCli("getnewaddress", "\"\" \"pq\"") +
                    "\nGet a Bitcoin-style bech32 address\n"
...
RPCHelpMan getrawchangeaddress()
{
    return RPCHelpMan{"getrawchangeaddress",
                ...
                {
                    {"address_type", RPCArg::Type::STR, RPCArg::DefaultHint{"set by -changetype"}, "The address type to use. Options are \"legacy\", \"p2sh-segwit\", \"bech32\", \"bech32m\". For PQ/Dilithium addresses, use getnewdilithiumaddress."},
                },
                RPCResult{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (!request.params[0].isNull()) {
        ...
        if (!parsed) {
            ...
            throw JSONRPCError(RPC_INVALID_PARAMETER, "Legacy wallets cannot provide bech32m addresses");
        } else if (parsed.value() == OutputType::DILITHIUM_LEGACY || parsed.value() == OutputType::DILITHIUM_BECH32) {
            throw JSONRPCError(RPC_INVALID_PARAMETER, "pq/dilithium addresses are not supported via getrawchangeaddress. Use getnewdilithiumaddress.");
        }
        output_type = parsed.value();
    }
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    RPCFillableSigningProvider provider;
    CScript inner;
    ...
    auto scripts_map = provider.GetScriptsMap();
    for (const auto& [id, script] : scripts_map) {
...
public:
    ...
    {
        UniValue obj(UniValue::VOBJ);
        // Note: SigningProvider doesn't currently support Dilithium pubkeys
        // This can be enhanced later if provider interface is extended
        return obj;
    }

    // NOLINTNEXTLINE(misc-no-recursion)
    UniValue operator()(const DilithiumScriptHash& scripthash) const
    {
        UniValue obj(UniValue::VOBJ);
        CScript subscript;
        // Note: SigningProvider doesn't currently support Dilithium scripts
        // This can be enhanced later if provider interface is extended
        return obj;
    }
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (provider) {
        ...
        bool inferred_solvable = inferred->IsSolvable();
        solvable = inferred_solvable;
        ...
        // Only push desc when it comes from inferred->IsSolvable() (NOT from the Dilithium fallback)
        if (inferred_solvable) {
