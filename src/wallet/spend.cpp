...
namespace wallet {
/**
 * Check if a script contains PQ opcodes (OP_PQCHECKSIG, OP_PQCHECKSIGVERIFY, OP_PQCHECKSIGADD,
 * OP_CHECKSIGDILITHIUM, OP_CHECKSIGDILITHIUMVERIFY).
 */
...
int CalculateMaximumSignedInputSize(const CTxOut& txout, const COutPoint outpoint, const SigningProvider* provider, bool can_grind_r, const CCoinControl* coin_control, int witness_discount_scale)
{
    ...
    // Fallback for Dilithium outputs: estimate size if provider can solve the script
    std::vector<std::vector<unsigned char>> solutions;
    TxoutType whichType = Solver(txout.scriptPubKey, solutions);
    if (whichType == TxoutType::DILITHIUM_PUBKEYHASH || whichType == TxoutType::QBITX_DILITHIUM) {
        // Check if provider has the Dilithium key to solve this script
        if (solutions.size() > 0 && solutions[0].size() == 20) {
            ...
            if (provider->GetDilithiumPubKey(dilPkhash, dilPubKey)) {
                // Provider can solve this Dilithium script - return estimated size
                // For Dilithium P2PKH: scriptSig = <sig+hashtype> <pubkey>
                ...
                const int64_t scriptsig_len = sig_push_overhead + sig_size + pubkey_push_overhead + pubkey_size;
                                                  ...
                                                  int witness_discount_scale)
{
    ...
    std::vector<std::vector<unsigned char>> solutions;
    TxoutType whichType = Solver(txo.scriptPubKey, solutions);
    if (whichType == TxoutType::DILITHIUM_PUBKEYHASH || whichType == TxoutType::QBITX_DILITHIUM) {
        // For Dilithium P2PKH: scriptSig = <sig+hashtype> <pubkey>
        // Signature: DILITHIUM_SIGNATUREBYTES + 1 (hashtype byte)
        // Public key: DILITHIUM_PUBLICKEYBYTES
        // Push overhead: 1 byte (1-75), 2 bytes (76-255), 3 bytes (256+)
        const int64_t sig_size = DILITHIUM_SIGNATUREBYTES + 1;
        const int64_t pubkey_size = DILITHIUM_PUBLICKEYBYTES;
        const int64_t sig_push_overhead = (sig_size > 255) ? 3 : (sig_size > 75) ? 2 : 1;
        const int64_t pubkey_push_overhead = (pubkey_size > 255) ? 3 : (pubkey_size > 75) ? 2 : 1;
        ...
        bool sign) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    // Check if any selected inputs are PQ/Dilithium-related
    bool has_pq_inputs = false;
    for (const auto& coin : selected_coins) {
        std::vector<std::vector<unsigned char>> solutions;
        TxoutType whichType = Solver(coin->txout.scriptPubKey, solutions);
        if (whichType == TxoutType::DILITHIUM_PUBKEY ||
    ...
    int nBytes = -1;
    TxSize tx_sizes{0, 0}; // Declare in outer scope for later use in size check
    
    // For PQ/Dilithium inputs, always use real signing to get accurate size
    // Standard estimation underestimates PQ spends, causing "Fee needed > fee paid" errors
    if (has_pq_inputs) {
        // Make a copy of the transaction for signing
        CMutableTransaction tmp_tx = txNew;
        // Sign the transaction to get accurate size
        if (!wallet.DilithiumSignTransaction(tmp_tx)) {
    ...
    // Check if any input spends a PQ script (contains OP_PQCHECKSIG/OP_PQCHECKSIGVERIFY/OP_PQCHECKSIGADD)
    // Only call DilithiumSignTransaction if PQ scripts are present
    if (sign) {
        if (has_pq_inputs) {
            // PQ inputs require Dilithium signing
            if (!wallet.DilithiumSignTransaction(txNew)) {
                return util::Error{_("Dilithium signing failed")};
            }
        } else {
            // Standard inputs use regular wallet signing
            // Build coins map from selected_coins for SignTransaction
            std::map<COutPoint, Coin> coins;
            for (const auto& coin : selected_coins) {
                coins[coin->outpoint] = Coin(coin->txout, coin->depth, coin->spendable);
            }
            std::map<int, bilingual_str> input_errors;
            if (!wallet.SignTransaction(txNew, coins, SIGHASH_DEFAULT, input_errors)) {
                if (!input_errors.empty()) {
                    return util::Error{input_errors.begin()->second};
                }
