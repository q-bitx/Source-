...
bool BerkeleyRODatabase::Backup(const std::string& dest) const
{
    ...
    fs::path dst(PathFromString(dest));
    ...
    try {
        ...
        LogPrintf("copied %s to %s\n", PathToString(m_filepath), PathToString(dst));
        return true;
    } catch (const fs::filesystem_error& e) {
        LogPrintf("error copying %s to %s - %s\n", PathToString(m_filepath), PathToString(dst), fsbridge::get_filesystem_error_message(e));
        return false;
    }
}
