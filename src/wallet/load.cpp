...
bool VerifyWallets(WalletContext& context)
{
    ...
    if (args.IsArgSet("-walletdir")) {
        ...
        if (error || !fs::exists(canonical_wallet_dir)) {
            chain.initError(strprintf(_("Specified -walletdir \"%s\" does not exist"), PathToString(wallet_dir)));
            return false;
        } else if (!fs::is_directory(canonical_wallet_dir)) {
            chain.initError(strprintf(_("Specified -walletdir \"%s\" is not a directory"), PathToString(wallet_dir)));
            return false;
        // The canonical path transforms relative paths into absolute ones, so we check the non-canonical version
        } else if (!wallet_dir.is_absolute()) {
            chain.initError(strprintf(_("Specified -walletdir \"%s\" is a relative path"), PathToString(wallet_dir)));
            return false;
        }
        args.ForceSetArg("-walletdir", PathToString(canonical_wallet_dir));
    }

    LogPrintf("Using wallet directory %s\n", PathToString(GetWalletDir()));
    ...
    for (const auto& wallet : chain.getSettingsList("wallet")) {
        ...
        const fs::path path = fsbridge::AbsPathJoin(GetWalletDir(), PathFromString(wallet_file));
...
bool LoadWallets(WalletContext& context)
{
    ...
    try {
        ...
        for (const auto& wallet : chain.getSettingsList("wallet")) {
            ...
            if (!wallet_paths.insert(PathFromString(name)).second) {
                continue;
            }
            DatabaseOptions options;
            DatabaseStatus status;
