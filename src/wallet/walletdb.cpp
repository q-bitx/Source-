...
const std::string DILITHIUM_KEY{"dilsk"};
const std::string DILITHIUM_PUBKEY{"dilpk"};
// Dilithium key storage keys for descriptor wallets (using different names to avoid conflicts)
const std::string DILITHIUM_CRYPTED_KEY{"dilithiumckey"};
const std::string DILITHIUM_KEYMETA{"dilithiumkeymeta"};
const std::unordered_set<std::string> LEGACY_TYPES{CRYPTED_KEY, CSCRIPT, DEFAULTKEY, HDCHAIN, KEYMETA, KEY, OLD_KEY, POOL, WATCHMETA, WATCHS, DILITHIUM_PUBKEY, DILITHIUM_KEY, DILITHIUM_CRYPTED_KEY, DILITHIUM_KEYMETA};
} // namespace DBKeys

//
// WalletBatch
//
bool WalletBatch::WritePQPrivateKey(const std::vector<uint8_t>& key)
{
    return WriteIC(std::string("pqprivkey"), key);
}
...
//bool CWalletDB::WritePQPrivateKey(const std::vector<uint8_t>& priv) {
//    return Write(std::string("pqpriv"), priv);
//}

//bool CWalletDB::ReadPQPrivateKey(std::vector<uint8_t>& priv) {
  //  return Read(std::string("pqpriv"), priv);
//}

bool WalletBatch::WriteDilithiumKey(const CKeyID& keyid, const std::vector<unsigned char>& vchPubKey, const std::vector<unsigned char>& vchPrivKey)
{
    // Store public key
    if (!WriteIC(std::make_pair(DBKeys::DILITHIUM_PUBKEY, keyid), vchPubKey, false)) {
        return false;
    }
    // Store private key with hash for integrity check
    std::vector<unsigned char> vchKey;
    vchKey.reserve(vchPubKey.size() + vchPrivKey.size());
    vchKey.insert(vchKey.end(), vchPubKey.begin(), vchPubKey.end());
    vchKey.insert(vchKey.end(), vchPrivKey.begin(), vchPrivKey.end());
    return WriteIC(std::make_pair(DBKeys::DILITHIUM_KEY, keyid), std::make_pair(vchPrivKey, Hash(vchKey)), false);
}

bool WalletBatch::WriteDilithiumPubKey(const CKeyID& keyID, const std::vector<unsigned char>& vchPubKey)
{
    return WriteIC(std::make_pair(DBKeys::DILITHIUM_PUBKEY, keyID), vchPubKey, false);
}

// Dilithium key storage methods for descriptor wallets
...
bool LoadDilithiumPubKey(CWallet* pwallet, DataStream& ssKey, DataStream& ssValue, std::string& strErr)
{
    LOCK(pwallet->cs_wallet);
    try {
        CKeyID keyid;
        ssKey >> keyid;
        if (keyid.IsNull()) {
            strErr = "Error reading wallet database: CKeyID corrupt";
            return false;
        }
        std::vector<unsigned char> vchPubKey;
        ssValue >> vchPubKey;
        // Store public key in wallet map (private key will be loaded separately)
        pwallet->map_dilithium_pub[keyid] = vchPubKey;
    } catch (const std::exception& e) {
        if (strErr.empty()) {
            strErr = e.what();
        }
...
bool LoadDilithiumKey(CWallet* pwallet, DataStream& ssKey, DataStream& ssValue, std::string& strErr)
{
    ...
    try {
        ...
        std::pair<std::vector<unsigned char>, uint256> privkey_hash;
        ssValue >> privkey_hash;
        std::vector<unsigned char> vchPrivKey = privkey_hash.first;
        uint256 hash = privkey_hash.second;
        
        // Get public key from wallet map, or read from DB if not loaded yet
        std::vector<unsigned char> vchPubKey;
        bool is_descriptor = pwallet->IsWalletFlagSet(WALLET_FLAG_DESCRIPTORS);
        if (pwallet->map_dilithium_pub.count(keyid) > 0) {
            vchPubKey = pwallet->map_dilithium_pub[keyid];
        } else {
            // Pubkey not loaded yet, try to read it from DB
            std::unique_ptr<DatabaseBatch> db_batch = pwallet->GetDatabase().MakeBatch();
            if (!db_batch->Read(std::make_pair(DBKeys::DILITHIUM_PUBKEY, keyid), vchPubKey)) {
                strErr = "Error reading wallet database: Dilithium public key not found";
                return false;
            }
            // Store it in the map for future use
            pwallet->map_dilithium_pub[keyid] = vchPubKey;
        }
        // Verify hash based on wallet type
        // For descriptor wallets using WriteDilithiumKeyByID: hash = Hash(keyID + privkey)
        // For legacy wallets using WriteDilithiumKey: hash = Hash(pubkey + privkey)
        std::vector<unsigned char> vchKey;
        if (is_descriptor) {
            // Descriptor format: hash = Hash(keyID + privkey)
            vchKey.reserve(keyid.size() + vchPrivKey.size());
            vchKey.insert(vchKey.end(), keyid.begin(), keyid.end());
            vchKey.insert(vchKey.end(), vchPrivKey.begin(), vchPrivKey.end());
        } else {
            // Legacy format: hash = Hash(pubkey + privkey)
            vchKey.reserve(vchPubKey.size() + vchPrivKey.size());
            vchKey.insert(vchKey.end(), vchPubKey.begin(), vchPubKey.end());
            vchKey.insert(vchKey.end(), vchPrivKey.begin(), vchPrivKey.end());
        }
        ...
        // For descriptor wallets, load into DescriptorScriptPubKeyMan
        if (is_descriptor) {
            // Verify keyid matches the Dilithium pubkey (recompute to ensure correctness)
            CDilithiumPubKey dilPubKey(vchPubKey);
            if (!dilPubKey.IsValid()) {
                strErr = "Error reading wallet database: Invalid Dilithium public key";
                return false;
            }
            uint160 computed_keyid_hash = dilPubKey.GetID();
            CKeyID computed_keyid = CKeyID(computed_keyid_hash);
            if (computed_keyid != keyid) {
                // Keyid mismatch - use the computed one from pubkey (this is the correct one)
                LogPrintf("WARNING: Dilithium keyid mismatch in DB. DB keyid=%s, computed from pubkey=%s. Using computed keyid.\n",
                    HexStr(keyid), HexStr(computed_keyid));
            ...
            for (ScriptPubKeyMan* spk_man : pwallet->GetAllScriptPubKeyMans()) {
                ...
                if (desc_spk_man) {
                    if (!desc_spk_man->LoadDilithiumKeyFromDB(keyid, vchPrivKey, vchPubKey)) {
                        strErr = "Error reading wallet database: Failed to load Dilithium key into descriptor wallet";
                        return false;
                    }
                    // Also update wallet's map_dilithium_pub for compatibility
                    pwallet->map_dilithium_pub[keyid] = vchPubKey;
                    break;
                }
            }
        } else {
            // For legacy wallets, use legacy storage
            pwallet->map_dilithium_priv[keyid] = vchPrivKey;
            pwallet->map_dilithium_pub[keyid] = vchPubKey;
        }
    } catch (const std::exception& e) {
        if (strErr.empty()) {
...
bool LoadDilithiumCryptedKey(CWallet* pwallet, DataStream& ssKey, DataStream& ssValue, std::string& strErr)
{
    LOCK(pwallet->cs_wallet);
    ...
    try {
        ...
        std::pair<std::vector<unsigned char>, uint256> crypted_checksum;
        ssValue >> crypted_checksum;
        std::vector<unsigned char> vchCryptedSecret = crypted_checksum.first;
        uint256 checksum = crypted_checksum.second;
        
        // Verify checksum
        if (Hash(vchCryptedSecret) != checksum) {
            strErr = "Error reading wallet database: Dilithium encrypted key corrupt";
            return false;
        }
        
        // Get public key from wallet map or DB
        std::vector<unsigned char> vchPubKey;
        if (pwallet->map_dilithium_pub.count(keyid) > 0) {
            vchPubKey = pwallet->map_dilithium_pub[keyid];
        } else {
        ...
        if (is_descriptor) {
            for (ScriptPubKeyMan* spk_man : pwallet->GetAllScriptPubKeyMans()) {
                DescriptorScriptPubKeyMan* desc_spk_man = dynamic_cast<DescriptorScriptPubKeyMan*>(spk_man);
                if (desc_spk_man) {
                    if (!desc_spk_man->LoadCryptedDilithiumKey(keyid, vchPubKey, vchCryptedSecret)) {
                        strErr = "Error reading wallet database: Failed to load encrypted Dilithium key";
                        return false;
                    }
                    break;
                }
...
bool LoadDilithiumKeyMeta(CWallet* pwallet, DataStream& ssKey, DataStream& ssValue, std::string& strErr)
{
    LOCK(pwallet->cs_wallet);
    try {
        CKeyID keyid;
        ...
        // Metadata is already associated with the key via WriteDilithiumKeyByID/WriteCryptedDilithiumKeyByID
        // This loader ensures the record is processed during wallet load
        (void)keyMeta;
    } catch (const std::exception& e) {
        if (strErr.empty()) {
            strErr = e.what();
...
static LoadResult LoadRecords(CWallet* pwallet, DatabaseBatch& batch, const std::string& key, DataStream& prefix, LoadFunc load_func)
{
    ...
    std::string last_successful_type = "none";
    std::string wallet_file = PathToString(pwallet->GetDatabase().Filename());
    ...
    while (true) {
        ...
        // Capture key info before deserializing
        size_t key_len = ssKey.size();
        size_t hex_prefix_len = std::min<size_t>(key_len, 80);
        std::span<const std::byte> key_span(ssKey.data(), key_len);
        std::string key_hex_prefix = HexStr(MakeUCharSpan(key_span).first(hex_prefix_len));
        ...
        try {
            ...
            err_msg = strprintf("exception extracting type: %s", e.what());
            LogPrintf("[wallet] CORRUPT record: wallet=%s type=%s key_len=%zu key_hex_prefix=%s last_ok=%s err=%s\n",
                     wallet_file, key, key_len, key_hex_prefix, last_successful_type, err_msg);
            result.m_result = DBErrors::CORRUPT;
            return result;
        }
        ...
        DBErrors record_res;
        try {
            record_res = load_func(pwallet, ssKey, ssValue, error);
        } catch (const std::exception& e) {
            err_msg = strprintf("exception in load_func: %s", e.what());
            LogPrintf("[wallet] CORRUPT record: wallet=%s type=%s key_len=%zu key_hex_prefix=%s last_ok=%s err=%s\n",
                     wallet_file, type, key_len, key_hex_prefix, last_successful_type, err_msg);
            result.m_result = DBErrors::CORRUPT;
            return result;
        }
        
        if (record_res != DBErrors::LOAD_OK) {
            std::string err_str = error.empty() ? "load_func returned error" : error;
            LogPrintf("[wallet] CORRUPT record: wallet=%s type=%s key_len=%zu key_hex_prefix=%s last_ok=%s err=%s\n",
                     wallet_file, type, key_len, key_hex_prefix, last_successful_type, err_str);
            pwallet->WalletLogPrintf("%s\n", error);
        } else {
            last_successful_type = type;
        }
...
static DBErrors LoadLegacyWalletRecords(CWallet* pwallet, DatabaseBatch& batch, int last_client) EXCLUSIVE_LOCKS_REQUIRED(pwallet->cs_wallet)
{
    ...
    // Dilithium (PQ) key types are explicitly allowed in descriptor wallets as an intentional PQ extension
    if (pwallet->IsWalletFlagSet(WALLET_FLAG_DESCRIPTORS)) {
        // Create a set of legacy types excluding Dilithium types (which are allowed in descriptor wallets)
        // Whitelist all known Dilithium DBKeys to prevent false positives
        std::unordered_set<std::string> dilithium_keys{
            DBKeys::DILITHIUM_KEY,           // "dilsk"
            DBKeys::DILITHIUM_PUBKEY,        // "dilpk"
            DBKeys::DILITHIUM_CRYPTED_KEY,   // "dilithiumckey"
            DBKeys::DILITHIUM_KEYMETA        // "dilithiumkeymeta"
        };
        
        std::unordered_set<std::string> legacy_types_excluding_dilithium;
        for (const auto& type : DBKeys::LEGACY_TYPES) {
            // Exclude all Dilithium-related keys from legacy check
            if (dilithium_keys.find(type) == dilithium_keys.end()) {
                legacy_types_excluding_dilithium.insert(type);
            }
        }
        
        for (const auto& type : legacy_types_excluding_dilithium) {
            DataStream key;
            DataStream value{};
            ...
            if (status != DatabaseCursor::Status::DONE) {
                ...
                LogPrintf("Descriptor wallet legacy entry detected: type=%s wallet=%s descriptors=true\n", type, pwallet->GetName());
    ...
    // Load Dilithium public keys first
    LoadResult dilpubkey_res = LoadRecords(pwallet, batch, DBKeys::DILITHIUM_PUBKEY,
        [] (CWallet* pwallet, DataStream& key, DataStream& value, std::string& err) {
        return LoadDilithiumPubKey(pwallet, key, value, err) ? DBErrors::LOAD_OK : DBErrors::CORRUPT;
    });
    result = std::max(result, dilpubkey_res.m_result);

    // Load Dilithium private keys
    LoadResult dilkey_res = LoadRecords(pwallet, batch, DBKeys::DILITHIUM_KEY,
        [] (CWallet* pwallet, DataStream& key, DataStream& value, std::string& err) {
        return LoadDilithiumKey(pwallet, key, value, err) ? DBErrors::LOAD_OK : DBErrors::CORRUPT;
    });
    result = std::max(result, dilkey_res.m_result);

    // Load Dilithium encrypted keys
    LoadResult dilcryptedkey_res = LoadRecords(pwallet, batch, DBKeys::DILITHIUM_CRYPTED_KEY,
        [] (CWallet* pwallet, DataStream& key, DataStream& value, std::string& err) {
        return LoadDilithiumCryptedKey(pwallet, key, value, err) ? DBErrors::LOAD_OK : DBErrors::CORRUPT;
    });
    result = std::max(result, dilcryptedkey_res.m_result);

    // Load Dilithium key metadata
    LoadResult dilkeymeta_res = LoadRecords(pwallet, batch, DBKeys::DILITHIUM_KEYMETA,
        [] (CWallet* pwallet, DataStream& key, DataStream& value, std::string& err) {
        return LoadDilithiumKeyMeta(pwallet, key, value, err) ? DBErrors::LOAD_OK : DBErrors::CORRUPT;
    });
    result = std::max(result, dilkeymeta_res.m_result);
...
static DBErrors LoadActiveSPKMs(CWallet* pwallet, DatabaseBatch& batch) EXCLUSIVE_LOCKS_REQUIRED(pwallet->cs_wallet)
{
    ...
    for (const auto& spk_key : {DBKeys::ACTIVEEXTERNALSPK, DBKeys::ACTIVEINTERNALSPK}) {
        LoadResult spkm_res = LoadRecords(pwallet, batch, spk_key,
            ...
            // Check if the ScriptPubKeyMan ID exists before trying to load it
            // This handles cases where wallet.dat contains entries for output types
            // that reference descriptor IDs not present in the current build
            if (pwallet->GetScriptPubKeyMan(id) == nullptr) {
                LogPrintf("[wallet] Skipping active ScriptPubKeyMan record: id=%s output_type=%d internal=%d (ScriptPubKeyMan not found, may be unsupported output type)\n",
                         id.ToString(), static_cast<int>(output_type), internal ? 1 : 0);
                return DBErrors::LOAD_OK;
            }
            ...
            try {
                ...
                LogPrintf("[wallet] Skipping active ScriptPubKeyMan record: id=%s output_type=%d internal=%d (exception: %s)\n",
                         id.ToString(), static_cast<int>(output_type), internal ? 1 : 0, e.what());
                return DBErrors::LOAD_OK;
            }
            return DBErrors::LOAD_OK;
        });
...
std::unique_ptr<WalletDatabase> MakeDatabase(const fs::path& path, const DatabaseOptions& options, DatabaseStatus& status, bilingual_str& error)
{
    ...
    try {
        ...
        error = Untranslated(strprintf("Failed to access database path '%s': %s", PathToString(path), fsbridge::get_filesystem_error_message(e)));
    ...
    if (exists) {
        ...
        if (IsSQLiteFile(SQLiteDataFile(path))) {
            if (format) {
                error = Untranslated(strprintf("Failed to load database path '%s'. Data is in ambiguous format.", PathToString(path)));
        ...
        error = Untranslated(strprintf("Failed to load database path '%s'. Path does not exist.", PathToString(path)));
        status = DatabaseStatus::FAILED_NOT_FOUND;
        return nullptr;
    }

    if (!format && options.require_existing) {
        error = Untranslated(strprintf("Failed to load database path '%s'. Data is not in recognized format.", PathToString(path)));
        status = DatabaseStatus::FAILED_BAD_FORMAT;
        return nullptr;
    }

    if (format && options.require_create) {
        error = Untranslated(strprintf("Failed to create database path '%s'. Database already exists.", PathToString(path)));
    ...
    if (format && options.require_format && format != options.require_format) {
        error = Untranslated(strprintf("Failed to load database path '%s'. Data is not in required format.", PathToString(path)));
        status = DatabaseStatus::FAILED_BAD_FORMAT;
        return nullptr;
    }
...
#endif
    {
        error = Untranslated(strprintf("Failed to open database path '%s'. Build does not support Berkeley DB database format.", PathToString(path)));
        status = DatabaseStatus::FAILED_BAD_FORMAT;
        return nullptr;
    }
}
