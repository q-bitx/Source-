...
std::shared_ptr<CWallet> RestoreWallet(
    ...
    bool load_after_restore)
{
    ...
    try {
        ...
        if (fs::exists(wallet_path) || !TryCreateDirectories(wallet_path)) {
            error = Untranslated(strprintf("Failed to create database path '%s'. Database already exists.", PathToString(wallet_path)));
...
wallet::DBErrors wallet::CWallet::ReorderTransactions()
{
    return wallet::DBErrors::LOAD_OK;
}
...
// Helper function to check if a Dilithium key exists in any ScriptPubKeyMan
static bool HaveDilithiumKeyInAnySPKM(const CWallet* pwallet, const CKeyID& keyid)
{
    ...
    for (ScriptPubKeyMan* spkm : pwallet->GetAllScriptPubKeyMans()) {
        ...
        if (desc_spkm) {
            ...
            if (desc_spkm->HaveDilithiumKey(keyid)) {
                return true;
            }
        }
    }
...
bool CWallet::HaveDilithiumKey(const CKeyID& keyid) const
{
    AssertLockHeld(cs_wallet);
    return HaveDilithiumKeyInAnySPKM(this, keyid);
}
...
isminetype CWallet::IsMine(const CTxDestination& dest) const
{
    ...
    // Check Dilithium addresses directly
    if (const DilithiumPKHash* dilithium_pkhash = std::get_if<DilithiumPKHash>(&dest)) {
        CKeyID keyid = CKeyID(uint160(*dilithium_pkhash));
...
isminetype CWallet::IsMine(const CScript& script) const
{
    ...
    // Check Dilithium P2PKH scripts directly
...
{
    LOCK(cs_wallet);
    // Check Dilithium types directly (they don't use ScriptPubKeyMan)
    if (!internal && !map_dilithium_priv.empty()) {
        return true;
    }
    if (m_spk_managers.empty()) return false;
    for (OutputType t : WALLET_OUTPUT_TYPES) {
...
void CWallet::InitWalletFlags(uint64_t flags)
{
    ...
    // Handle case where flags are already set (e.g., wallet was loaded from database)
    if (m_wallet_flags != 0) {
        if (m_wallet_flags == flags) {
            // Flags already match - idempotent call, just return
            WalletLogPrintf("InitWalletFlags: flags already set to 0x%x, skipping\n", flags);
            return;
        } else {
            // Flags mismatch - this is an error
            WalletLogPrintf("Error: InitWalletFlags called with flags 0x%x but wallet already has flags 0x%x\n", flags, m_wallet_flags);
            throw std::runtime_error(strprintf("Cannot initialize wallet flags: wallet already has flags 0x%x, attempted to set 0x%x", m_wallet_flags, flags));
        }
    }

    // This should only be used once, when creating a new wallet - so current flags are expected to be blank
    // (We've already handled the non-zero case above)

    if (!WalletBatch(GetDatabase()).WriteWalletFlags(flags)) {
        throw std::runtime_error(std::string(__func__) + ": writing wallet flags failed");
    }

    if (!LoadWalletFlags(flags)) {
        throw std::runtime_error(std::string(__func__) + ": loading wallet flags failed");
    }
}
...
bool wallet::CWallet::DilithiumSignTransaction(CMutableTransaction& tx) const
{
    PQPrivateKey pqkey;

    if (!LoadPQPrivateKey(pqkey)) {
        LogPrintf("Failed to load Dilithium private key from wallet\n");
        return false;
    }

    //CDataStream ss(std::vector<uint8_t>(), SER_NETWORK, PROTOCOL_VERSION);
   DataStream ss;
    SerializeTransaction(tx, ss, TransactionSerParams{});
// TransactionSerParams{SER_NETWORK, PROTOCOL_VERSION});
    //SerializeTransaction(tx, ss, TransactionSerParams{});
    std::vector<uint8_t> tx_data(ss.size());
    std::memcpy(tx_data.data(), ss.data(), ss.size());
    for (auto& input : tx.vin) {
        std::vector<uint8_t> sig;
        if (!PQ_Sign(pqkey, tx_data, sig)) {
            LogPrintf("Dilithium signing failed\n");
            return false;
        }
...
bool CWallet::LoadPQPrivateKey(PQPrivateKey& key_out) const {
    if (pq_private_key.empty()) return false;
    key_out = PQPrivateKey(pq_private_key);
    return true;
}

bool CWallet::SetPQPrivateKey(const PQPrivateKey& key_in) {
    pq_private_key = key_in.data;

    WalletBatch batch(*m_database, /* fFlushOnClose= */ true);
    return batch.WritePQPrivateKey(pq_private_key);
}
...
util::Result<CTxDestination> CWallet::GetNewDestination(const OutputType type, const std::string label)
{
    ...
    if (type == OutputType::DILITHIUM_LEGACY) {
        ...
        if (!PQ_GenerateKeypair(pubkey_bytes, privkey_bytes)) {
            return util::Error{_("Error: Failed to generate Dilithium keypair")};
        }
        
        // Create CDilithiumPubKey and compute CKeyID
        CDilithiumPubKey dilithium_pubkey(pubkey_bytes);
        if (!dilithium_pubkey.IsValid()) {
            return util::Error{_("Error: Invalid Dilithium public key generated")};
        }
        
        uint160 keyid_hash = dilithium_pubkey.GetID();
        ...
        if (IsWalletFlagSet(WALLET_FLAG_DESCRIPTORS)) {
            ...
            if (!spk_man) {
                ...
                if (IsCrypted()) {
                    ...
                    if (!spk_manager->CheckDecryptionKey(vMasterKey) && !spk_manager->Encrypt(vMasterKey, &batch)) {
                        return util::Error{_("Error: Could not encrypt Dilithium key storage")};
                    }
                }
            ...
            if (!desc_spk_man) {
                return util::Error{_("Error: Script pubkey manager is not a descriptor manager")};
            }
            if (!desc_spk_man->AddDilithiumKeyPubKey(keyid, privkey_bytes, pubkey_bytes)) {
                return util::Error{_("Error: Failed to add Dilithium key to descriptor wallet")};
            }
            // Also populate map_dilithium_pub for compatibility and immediate access
            map_dilithium_pub[keyid] = pubkey_bytes;
        } else {
            // For legacy wallets, use legacy storage
            map_dilithium_pub[keyid] = pubkey_bytes;
            map_dilithium_priv[keyid] = privkey_bytes;
            
            // Save to database using legacy method
            WalletBatch batch(GetDatabase());
            if (!batch.WriteDilithiumKey(keyid, pubkey_bytes, privkey_bytes)) {
                return util::Error{_("Error: Failed to write Dilithium key to database")};
            }
        }
    ...
    // Validate that OutputType::LEGACY produces a standard P2PKH destination
    if (type == OutputType::LEGACY) {
        CTxDestination dest = *op_dest;
        // Ensure it's a PKHash (P2PKH address)
        if (!std::holds_alternative<PKHash>(dest)) {
            CScript scriptPubKey = GetScriptForDestination(dest);
            return util::Error{strprintf(_("Error: legacy output type produced non-standard destination. Script: %s"), HexStr(scriptPubKey))};
        }
        // Verify the scriptPubKey is a valid P2PKH script (76a914...88ac)
        CScript scriptPubKey = GetScriptForDestination(dest);
        std::vector<std::vector<unsigned char>> vSolutions;
        TxoutType whichType = Solver(scriptPubKey, vSolutions);
        if (whichType != TxoutType::PUBKEYHASH) {
            return util::Error{strprintf(_("Error: legacy output type produced non-standard script. Expected P2PKH (pubkeyhash), got %s. Script: %s"), GetTxnOutputType(whichType), HexStr(scriptPubKey))};
        }
        // Double-check ExtractDestination works
        CTxDestination check_dest;
        if (!ExtractDestination(scriptPubKey, check_dest)) {
            return util::Error{strprintf(_("Error: legacy output type produced non-standard script. Cannot extract destination. Script: %s"), HexStr(scriptPubKey))};
        }
        // Ensure it's still a PKHash after extraction
        if (!std::holds_alternative<PKHash>(check_dest)) {
            return util::Error{strprintf(_("Error: legacy output type produced non-standard destination after extraction. Script: %s"), HexStr(scriptPubKey))};
        }
    }
...
util::Result<void> CWallet::DisplayAddress(const CTxDestination& dest)
{
    // External signer functionality disabled in Q-BitX
    return util::Error{_("External signer is not supported in Q-BitX")};
}
...
static util::Result<fs::path> GetWalletPath(const std::string& name)
{
    ...
    const fs::path wallet_path = fsbridge::AbsPathJoin(GetWalletDir(), PathFromString(name));
    fs::file_type path_type = fs::symlink_status(wallet_path).type();
    if (!(path_type == fs::file_type::not_found || path_type == fs::file_type::directory ||
          (path_type == fs::file_type::symlink && fs::is_directory(wallet_path)) ||
          (path_type == fs::file_type::regular && PathFromString(name).filename() == PathFromString(name)))) {
        return util::Error{Untranslated(strprintf(
              "Invalid -wallet path '%s'. -wallet path should point to a directory where wallet.dat and "
              "database/log.?????????? files can be stored, a location where such a directory could be created, "
              "or (for backwards compatibility) the name of an existing data file in -walletdir (%s)",
              name, quoted(PathToString(GetWalletDir()))))};
    }
    return wallet_path;
}
...
std::shared_ptr<CWallet> CWallet::Create(WalletContext& context, const std::string& name, std::unique_ptr<WalletDatabase> database, uint64_t wallet_creation_flags, bilingual_str& error, std::vector<bilingual_str>& warnings)
{
    ...
    // Helper function to map DBErrors enum to string
    auto DBErrorsToString = [](DBErrors err) -> std::string {
        switch (err) {
            case DBErrors::LOAD_OK: return "LOAD_OK";
            case DBErrors::NEED_RESCAN: return "NEED_RESCAN";
            case DBErrors::NEED_REWRITE: return "NEED_REWRITE";
            case DBErrors::EXTERNAL_SIGNER_SUPPORT_REQUIRED: return "EXTERNAL_SIGNER_SUPPORT_REQUIRED";
            case DBErrors::NONCRITICAL_ERROR: return "NONCRITICAL_ERROR";
            case DBErrors::TOO_NEW: return "TOO_NEW";
            case DBErrors::UNKNOWN_DESCRIPTOR: return "UNKNOWN_DESCRIPTOR";
            case DBErrors::LOAD_FAIL: return "LOAD_FAIL";
            case DBErrors::UNEXPECTED_LEGACY_ENTRY: return "UNEXPECTED_LEGACY_ENTRY";
            case DBErrors::CORRUPT: return "CORRUPT";
    ...
    if (nLoadWalletRet != DBErrors::LOAD_OK) {
        errorCodeStr = DBErrorsToString(nLoadWalletRet);
        errorCodeInt = static_cast<int>(nLoadWalletRet);
        LogPrintf("[wallet] LoadWallet failed for %s: code=%d (%s)\n", walletFile, errorCodeInt, errorCodeStr);
        if (nLoadWalletRet == DBErrors::CORRUPT) {
            error = strprintf(_("Error loading %s: Wallet corrupted (code=%d, %s)"), walletFile, errorCodeInt, errorCodeStr);
            return nullptr;
        }
        else if (nLoadWalletRet == DBErrors::NONCRITICAL_ERROR)
        {
            ...
            error = strprintf(_("Error loading %s: Wallet requires newer version of %s (code=%d, %s)"), walletFile, CLIENT_NAME, errorCodeInt, errorCodeStr);
            return nullptr;
        }
        else if (nLoadWalletRet == DBErrors::EXTERNAL_SIGNER_SUPPORT_REQUIRED) {
            error = strprintf(_("Error loading %s: External signer wallet being loaded without external signer support compiled (code=%d, %s)"), walletFile, errorCodeInt, errorCodeStr);
            return nullptr;
        }
        else if (nLoadWalletRet == DBErrors::NEED_REWRITE)
        {
            error = strprintf(_("Wallet needed to be rewritten: restart %s to complete (code=%d, %s)"), CLIENT_NAME, errorCodeInt, errorCodeStr);
            return nullptr;
        } else if (nLoadWalletRet == DBErrors::NEED_RESCAN) {
            warnings.push_back(strprintf(_("Error reading %s! Transaction data may be missing or incorrect."
                                           " Rescanning wallet. (code=%d, %s)"), walletFile, errorCodeInt, errorCodeStr));
            rescan_required = true;
        } else if (nLoadWalletRet == DBErrors::UNKNOWN_DESCRIPTOR) {
            error = strprintf(_("Unrecognized descriptor found. Loading wallet %s\n\n"
                                "The wallet might had been created on a newer version.\n"
                                "Please try running the latest software version.\n (code=%d, %s)"), walletFile, errorCodeInt, errorCodeStr);
            return nullptr;
            ...
            error = strprintf(_("Error loading %s (code=%d, %s)"), walletFile, errorCodeInt, errorCodeStr);
            return nullptr;
        }
    }

    // This wallet is in its first run if there are no ScriptPubKeyMans and it isn't blank or no privkeys
    // Also check that flags haven't been loaded yet (wallet exists but is new)
    const bool fFirstRun = walletInstance->m_spk_managers.empty() &&
                     !walletInstance->IsWalletFlagSet(WALLET_FLAG_DISABLE_PRIVATE_KEYS) &&
                     !walletInstance->IsWalletFlagSet(WALLET_FLAG_BLANK_WALLET) &&
                     walletInstance->m_wallet_flags == 0;
    if (fFirstRun)
    {
        ...
        if ((wallet_creation_flags & WALLET_FLAG_EXTERNAL_SIGNER) || !(wallet_creation_flags & (WALLET_FLAG_DISABLE_PRIVATE_KEYS | WALLET_FLAG_BLANK_WALLET))) {
            ...
            if (walletInstance->IsWalletFlagSet(WALLET_FLAG_DESCRIPTORS)) {
                try {
                    ...
                    walletInstance->WalletLogPrintf("Descriptor ScriptPubKeyMans setup completed successfully\n");
                } catch (const std::exception& e) {
                    error = strprintf(_("Error setting up descriptor wallet: %s"), e.what());
                    return nullptr;
                }
...
std::set<ScriptPubKeyMan*> CWallet::GetScriptPubKeyMans(const CScript& script) const
{
    ...
    if (spkm && spkm->CanProvide(script, sigdata)) {
        spk_mans.insert(spkm);
        return spk_mans;
    }

    // For descriptor wallets, if cache miss, check all ScriptPubKeyMans
    if (IsWalletFlagSet(WALLET_FLAG_DESCRIPTORS)) {
        for (const auto& spk_man_pair : m_spk_managers) {
...
DescriptorScriptPubKeyMan& CWallet::LoadDescriptorScriptPubKeyMan(uint256 id, WalletDescriptor& desc)
{
    DescriptorScriptPubKeyMan* spk_manager = new DescriptorScriptPubKeyMan(*this, desc, m_keypool_size);
    AddScriptPubKeyMan(id, std::unique_ptr<ScriptPubKeyMan>(spk_manager));
    return *spk_manager;
}
...
void CWallet::SetupDescriptorScriptPubKeyMans()
{
    ...
    WalletLogPrintf("Setting up descriptor ScriptPubKeyMans\n");
    ...
    if (!IsWalletFlagSet(WALLET_FLAG_EXTERNAL_SIGNER)) {
        ...
        WalletLogPrintf("Descriptor ScriptPubKeyMans setup completed, keypool size: %u\n", GetKeyPoolSize());
    } else {
        // External signer support - not implemented in this fork
        throw std::runtime_error("Error: external signer wallets are not supported");
    }
}
...
bool CWallet::MigrateToSQLite(bilingual_str& error)
{
    ...
    fs::path db_path = PathFromString(m_database->Filename());
    m_database->Close();
    fs::remove(db_path);
    ...
    const fs::path wallet_path = fsbridge::AbsPathJoin(GetWalletDir(), PathFromString(m_name));
...
bool cDoMigration(CWallet& wallet, WalletContext& context, bilingual_str& error, MigrationResult& res) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
...
util::Result<MigrationResult> MigrateLegacyToDescriptor(std::shared_ptr<CWallet> local_wallet, const SecureString& passphrase, WalletContext& context, bool was_loaded)
{
    ...
    fs::path this_wallet_dir = fs::absolute(PathFromString(local_wallet->GetDatabase().Filename())).parent_path();
    fs::path backup_filename = PathFromString(strprintf("%s_%d.legacy.bak", (wallet_name.empty() ? "default_wallet" : wallet_name), GetTime()));
    fs::path backup_path = this_wallet_dir / backup_filename;
    if (!local_wallet->BackupWallet(PathToString(backup_path))) {
        if (was_loaded) {
            reload_wallet(local_wallet);
        }
...

    {
        ...
        if (!success) {
      	success = cDoMigration(*local_wallet, context, error, res);
        } else {
    ...
    if (success) {
        ...
        wallet_dirs.insert(PathFromString(local_wallet->GetDatabase().Filename()).parent_path());
        success = reload_wallet(local_wallet);
        res.wallet = local_wallet;
        res.wallet_name = wallet_name;
        if (success && res.watchonly_wallet) {
            // Reload watchonly
            wallet_dirs.insert(PathFromString(res.watchonly_wallet->GetDatabase().Filename()).parent_path());
            success = reload_wallet(res.watchonly_wallet);
        }
        if (success && res.solvables_wallet) {
            // Reload solvables
            wallet_dirs.insert(PathFromString(res.solvables_wallet->GetDatabase().Filename()).parent_path());
    ...
    if (!success) {
        ...
        for (std::shared_ptr<CWallet>& w : created_wallets) {
            wallet_dirs.emplace(PathFromString(w->GetDatabase().Filename()).parent_path());
        }
