...
IsMineResult IsMineInner(const LegacyDataSPKM& keystore, const CScript& scriptPubKey, IsMineSigVersion sigversion, bool recurse_scripthash=true)
{
   ...
   // case TxoutType::ANCHOR:
    ...
    case TxoutType::WITNESS_V1_TAPROOT:
        // Taproot outputs are not handled by standard keystore in this path
        // They require specialized handling elsewhere
        break;
    case TxoutType::QBITX_DILITHIUM:
    ...
    case TxoutType::DILITHIUM_MULTISIG:
        // Dilithium/PQ outputs are not handled by standard keystore in this path
        // They require specialized handling elsewhere
        break;
...
util::Result<CTxDestination> DescriptorScriptPubKeyMan::GetNewDestination(const OutputType type)
{
    ...
    }
    {
        ...
        if (type != *desc_addr_type) {
            ...
            return util::Error{strprintf(_("Error: Wallet does not support %s addresses. This wallet only supports %s addresses."), FormatOutputType(type), FormatOutputType(*desc_addr_type))};
        }
        ...
        if (!ExtractDestination(scripts_temp[0], dest)) {
            // Debug: log the script that failed to extract
            WalletLogPrintf("Error: Cannot extract destination from scriptPubKey: %s\n", HexStr(scripts_temp[0]));
            return util::Error{strprintf(_("Error: Cannot extract destination from the generated scriptpubkey. Script: %s"), HexStr(scripts_temp[0]))};
        }
...
bool DescriptorScriptPubKeyMan::AddDilithiumKeyWithDB(WalletBatch& batch, const CKeyID& keyid, const std::vector<unsigned char>& vchPrivKey, const std::vector<unsigned char>& vchPubKey)
{
    AssertLockHeld(cs_desc_man);
    assert(!m_storage.IsWalletFlagSet(WALLET_FLAG_DISABLE_PRIVATE_KEYS));
    ...
    if (m_storage.HasEncryptionKeys()) {
        ...
        std::vector<unsigned char> crypted_secret;
        CKeyingMaterial secret(vchPrivKey.begin(), vchPrivKey.end());
        if (!m_storage.WithEncryptionKey([&](const CKeyingMaterial& encryption_key) {
                return EncryptSecret(encryption_key, secret, uint256(keyid), crypted_secret);
            })) {
            return false;
        }
        m_map_crypted_dilithium_keys[keyid] = std::make_pair(vchPubKey, crypted_secret);
        // Write public key separately for encrypted keys too
        if (!batch.WriteDilithiumPubKey(keyid, vchPubKey)) {
            return false;
        }
        CKeyMetadata keyMeta(GetTime());
        return batch.WriteCryptedDilithiumKeyByID(keyid, crypted_secret, keyMeta);
    } else {
        m_map_dilithium_keys[keyid] = vchPrivKey;
        // Also write the public key separately so it can be retrieved later
        if (!batch.WriteDilithiumPubKey(keyid, vchPubKey)) {
            return false;
        }
        CKeyMetadata keyMeta(GetTime());
        return batch.WriteDilithiumKeyByID(keyid, vchPrivKey, keyMeta);
    }
}

bool DescriptorScriptPubKeyMan::AddDilithiumKeyPubKey(const CKeyID& keyid, const std::vector<unsigned char>& vchPrivKey, const std::vector<unsigned char>& vchPubKey)
{
    LOCK(cs_desc_man);
    WalletBatch batch(m_storage.GetDatabase());
    if (!AddDilithiumKeyWithDB(batch, keyid, vchPrivKey, vchPubKey)) {
        throw std::runtime_error(std::string(__func__) + ": writing Dilithium key failed");
    }
    return true;
}

bool DescriptorScriptPubKeyMan::LoadDilithiumKey(const CKeyID& keyid, const std::vector<unsigned char>& vchPrivKey, const std::vector<unsigned char>& vchPubKey)
{
    LOCK(cs_desc_man);
    // Verify keyid matches the Dilithium pubkey (derive correct keyid from pubkey)
    CDilithiumPubKey dilPubKey(vchPubKey);
    if (!dilPubKey.IsValid()) {
        LogPrintf("ERROR: Invalid Dilithium public key in LoadDilithiumKey\n");
        return false;
    }
    uint160 computed_keyid_hash = dilPubKey.GetID();
    CKeyID computed_keyid = CKeyID(computed_keyid_hash);
    
    // Use the computed keyid from the pubkey (this is the correct one)
    // Store the key with the correct keyid derived from the Dilithium pubkey
    CKeyID actual_keyid = computed_keyid;
    if (actual_keyid != keyid) {
        LogPrintf("WARNING: Dilithium keyid mismatch in LoadDilithiumKey. Passed keyid=%s, computed from pubkey=%s. Using computed keyid.\n",
            HexStr(keyid), HexStr(actual_keyid));
    }
    
    // Store the key in memory with the correct keyid
    m_map_dilithium_keys[actual_keyid] = vchPrivKey;
    return true;
}

bool DescriptorScriptPubKeyMan::LoadDilithiumKeyFromDB(const CKeyID& keyid, const std::vector<unsigned char>& vchPrivKey, const std::vector<unsigned char>& vchPubKey)
{
    return LoadDilithiumKey(keyid, vchPrivKey, vchPubKey);
}

bool DescriptorScriptPubKeyMan::LoadCryptedDilithiumKey(const CKeyID& keyid, const std::vector<unsigned char>& vchPubKey, const std::vector<unsigned char>& vchCryptedSecret)
{
    LOCK(cs_desc_man);
    // For descriptor wallets, store the encrypted key in memory
    m_map_crypted_dilithium_keys[keyid] = std::make_pair(vchPubKey, vchCryptedSecret);
    return true;
}

bool DescriptorScriptPubKeyMan::GetDilithiumKey_Locked(const CKeyID& keyid, std::vector<unsigned char>& vchPrivKey) const
{
    AssertLockHeld(cs_desc_man);
    ...
    if (mi != m_map_crypted_dilithium_keys.end()) {
        if (m_storage.HasEncryptionKeys()) {
            ...
            if (!m_storage.WithEncryptionKey([&](const CKeyingMaterial& encryption_key) {
                    return DecryptSecret(encryption_key, crypted_secret, uint256(keyid), secret);
                })) {
                return false;
            }
            vchPrivKey.assign(secret.begin(), secret.end());
            return true;
        }
    }
...
bool DescriptorScriptPubKeyMan::GetDilithiumKey(const CKeyID& keyid, std::vector<unsigned char>& vchPrivKey) const
{
    LOCK(cs_desc_man);
    return GetDilithiumKey_Locked(keyid, vchPrivKey);
}

bool DescriptorScriptPubKeyMan::GetDilithiumKeys(const CKeyID& keyid, std::vector<unsigned char>& vchPrivKey, std::vector<unsigned char>& vchPubKey) const
{
    LOCK(cs_desc_man);
    
    // Check encrypted keys first (they store public key in the pair)
    CryptedDilithiumKeyMap::const_iterator mi_enc = m_map_crypted_dilithium_keys.find(keyid);
    if (mi_enc != m_map_crypted_dilithium_keys.end()) {
        if (m_storage.HasEncryptionKeys()) {
            // Public key is stored in the first element of the pair
            vchPubKey = mi_enc->second.first;
            // Decrypt private key
            const std::vector<unsigned char>& crypted_secret = mi_enc->second.second;
            CKeyingMaterial secret;
            if (!m_storage.WithEncryptionKey([&](const CKeyingMaterial& encryption_key) {
                    return DecryptSecret(encryption_key, crypted_secret, uint256(keyid), secret);
                })) {
    ...
    // Check unencrypted keys (private key only, public key must be retrieved from wallet's map_dilithium_pub)
    DilithiumKeyMap::const_iterator mi = m_map_dilithium_keys.find(keyid);
    if (mi != m_map_dilithium_keys.end()) {
        vchPrivKey = mi->second;
        // For unencrypted keys in descriptor wallets, public key should be in CWallet::map_dilithium_pub
        // Return true for private key; caller must retrieve public key separately
        return true;
    }
...
bool DescriptorScriptPubKeyMan::HaveDilithiumKey_Locked(const CKeyID& keyid) const
{
    AssertLockHeld(cs_desc_man);
...
{
    LOCK(cs_desc_man);
    return HaveDilithiumKey_Locked(keyid);
}
...
std::unique_ptr<FlatSigningProvider> DescriptorScriptPubKeyMan::GetSigningProvider(const CScript& script, bool include_private) const
{
    ...
    // Check if this is a Dilithium P2PKH script and handle it separately
    std::vector<std::vector<unsigned char>> solutions;
    ...
    if (whichType == TxoutType::DILITHIUM_PUBKEYHASH && solutions.size() > 0 && solutions[0].size() == 20) {
        // Extract the CKeyID from the script hash
        CKeyID keyid = CKeyID(uint160(solutions[0]));
        
        // Check if we have the Dilithium key
        if (HaveDilithiumKey_Locked(keyid)) {
            ...
            if (GetDilithiumKeys(keyid, privkeyBytes, pubkeyBytes)) {
                // Populate the FlatSigningProvider with Dilithium keys
                DilithiumPKHash dilPkhash(keyid); // CKeyID and DilithiumPKHash both use uint160
    ...
    // Script not in map - search through descriptor indices to find which one produces this script
    // This can happen when UTXOs are received from external sources or cache is cleared
    // Search up to range_end which is the highest cached index
    int32_t search_limit = static_cast<int32_t>(std::max(m_wallet_descriptor.next_index, m_wallet_descriptor.range_end));
    for (int32_t i = 0; i <= search_limit; ++i) {
        std::vector<CScript> scripts_temp;
        FlatSigningProvider dummy_keys;
        if (m_wallet_descriptor.descriptor->ExpandFromCache(i, m_wallet_descriptor.cache, scripts_temp, dummy_keys)) {
            if (!scripts_temp.empty() && scripts_temp[0] == script) {
                // Found the matching index - get the SigningProvider with private keys if requested
                return GetSigningProvider(i, include_private);
            }
        }
    }

    // Script not found in any descriptor index
    return nullptr;
}
