...
//class CWallet {
//private:
  //  std::vector<uint8_t> pq_private_key;

//public:
  //  bool LoadPQPrivateKey(PQPrivateKey& key_out) const;
   // bool SetPQPrivateKey(const PQPrivateKey& key_in);
...
public:
    ...
    bool DilithiumSignTransaction(CMutableTransaction& tx) const;
    ...
    /** Dilithium key storage maps: CKeyID (20-byte Hash160 of pubkey) -> key data */
    std::map<CKeyID, std::vector<unsigned char>> map_dilithium_priv GUARDED_BY(cs_wallet);
    std::map<CKeyID, std::vector<unsigned char>> map_dilithium_pub GUARDED_BY(cs_wallet);
    ...
    bool HaveDilithiumKey(const CKeyID& keyid) const EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
    /**
