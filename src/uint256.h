...
consteval uint8_t LocalConstevalHexDigit(char c) {
...
public:
	...
	bool SetHex(std::string_view str);
};
...
consteval base_blob<BITS>::base_blob(std::string_view hex_str)
{
    ...
    for (auto& elem : m_data) {
       ...
       // elem = (ConstevalHexDigit(*(str_it++)) << 4) | lo;
        //elem = (util::ConstevalHexDigit(*(str_it++)) << 4) | lo;
        auto lo = LocalConstevalHexDigit(*(str_it++));
        elem = (LocalConstevalHexDigit(*(str_it++)) << 4) | lo; 
 }
}
/**
...
std::optional<uintN_t> FromHex(std::string_view str)
{
    if (!IsHex(str) || str.size() != uintN_t::size() * 2) return std::nullopt;
    uintN_t rv;
    rv.SetHexDeprecated(str);
    return rv;
}
