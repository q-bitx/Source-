...
//        std::string strUsage = CLIENT_NAME " RPC client version " + FormatFullVersion() + "\n";
        std::string strUsage = strprintf("%s RPC client version %s\n", CLIENT_NAME, FormatFullVersion());

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
//            strUsage += "\n"
            strUsage += std::string("The bitcoin-cli utility provides a command line interface to interact with a ")
         + CLIENT_NAME + " RPC server.\n";
    ...
    if (response.status == 0) {
        ...
        if (failedToGetAuthCookie) {
            throw std::runtime_error(strprintf(
                ...
                PathToString(gArgs.GetConfigFilePath())));
        } else {
