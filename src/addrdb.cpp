...
CBanDB::CBanDB(fs::path ban_list_path)
   ...
   //   m_banlist_json(ban_list_path / ".json")
    : m_banlist_dat(ban_list_path / "banlist.dat"),
      m_banlist_json(ban_list_path / "banlist.json")
...
//          return util::Error{strprintf(_("Failed to rename invalid peers.dat file. Please move or delete it and try again."))};
  //      }
    ...
    fs::path backup_path = path_addr;
    backup_path += ".bak";

    if (!RenameOver(path_addr, backup_path)) {
        return util::Error{strprintf(_("Failed to rename invalid peers.dat file. Please move or delete it and try again."))};
    }
    ...
    // Addrman can be in an inconsistent state after failure, reset it
        ...
        return util::Error{strprintf(_("Invalid or corrupt peers.dat (%s). If you believe this is a bug, please report it to %s. As a workaround, you can move the file (%s) out of the way (rename, move, or delete) to have a new one created on the next start."),
                                     e.what(), CLIENT_BUGREPORT, quoted(PathToString(path_addr)))};
    }
    return addrman;
}
...
std::vector<CAddress> ReadAnchors(const fs::path& anchors_db_path)
{
    ...
    try {
        ...
        LogPrintf("Loaded %i addresses from %s\n", anchors.size(), quoted(PathToString(anchors_db_path.filename())));
