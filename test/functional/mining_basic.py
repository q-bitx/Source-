...
class MiningTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("getblocktemplate: empty params defaults to segwit (for compatibility with miners)")
        tmpl_empty = node.getblocktemplate({})
        assert 'height' in tmpl_empty
        assert 'previousblockhash' in tmpl_empty
        
        self.log.info("getblocktemplate: explicitly missing segwit rule still fails")
        # Empty rules array should also default to segwit, so test with rules that explicitly exclude segwit
        # Note: This test verifies that if rules are provided but segwit is missing, it still fails
        # (though empty rules array will now default to segwit due to our change)
