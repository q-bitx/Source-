...
class RawTransactionsTest(BitcoinTestFramework):
    ...
    def createrawtransaction_tests(self):
        ...
        # Test `createrawtransaction` with scriptPubKey format
        self.log.info("Test createrawtransaction with scriptPubKey format")
        # Create a custom scriptPubKey (e.g., OP_QBITX <pubkey> OP_PQCHECKSIG)
        custom_script = CScript([OP_FALSE, OP_RETURN, b'PQ_TEST'])
        script_hex = custom_script.hex()
        amount = Decimal('0.001')
        
        # Create transaction with scriptPubKey format
        rawtx = self.nodes[0].createrawtransaction(
            inputs=[{'txid': TXID, 'vout': 0}],
            outputs=[{'scriptPubKey': script_hex, 'amount': float(amount)}]
        )
        
        # Decode and verify the scriptPubKey matches
        decrawtx = self.nodes[0].decoderawtransaction(rawtx)
        assert_equal(len(decrawtx['vout']), 1)
        assert_equal(decrawtx['vout'][0]['scriptPubKey']['hex'], script_hex)
        assert_equal(Decimal(decrawtx['vout'][0]['value']), amount)
        
        # Test with string amount
        rawtx2 = self.nodes[0].createrawtransaction(
            inputs=[{'txid': TXID, 'vout': 0}],
            outputs=[{'scriptPubKey': script_hex, 'amount': str(amount)}]
        )
        decrawtx2 = self.nodes[0].decoderawtransaction(rawtx2)
        assert_equal(decrawtx2['vout'][0]['scriptPubKey']['hex'], script_hex)
        
        # Test error cases
        assert_raises_rpc_error(-8, "Missing scriptPubKey field", 
                                self.nodes[0].createrawtransaction, 
                                [], [{'amount': 0.001}])
        assert_raises_rpc_error(-8, "Missing amount field", 
                                self.nodes[0].createrawtransaction, 
                                [], [{'scriptPubKey': script_hex}])
        assert_raises_rpc_error(-8, "Data must be hexadecimal string", 
                                self.nodes[0].createrawtransaction, 
                                [], [{'scriptPubKey': 'not_hex', 'amount': 0.001}])
