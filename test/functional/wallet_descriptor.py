...
class WalletDescriptorTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Test descriptor wallet signing
        self.log.info("Test descriptor wallet signing")
        self.test_descriptor_wallet_signing()
    ...
    def test_descriptor_wallet_signing(self):
        """Test that descriptor wallets can sign transactions correctly"""
        self.log.info("Test descriptor wallet signing with bech32 addresses")
        
        # Create a descriptor wallet
        wallet_name = "test_signing"
        self.nodes[0].createwallet(wallet_name=wallet_name, descriptors=True)
        wallet = self.nodes[0].get_wallet_rpc(wallet_name)
        
        # Get a bech32 address
        addr_bech32 = wallet.getnewaddress("", "bech32")
        self.log.info(f"Generated bech32 address: {addr_bech32}")
        
        # Mine 101 blocks to the bech32 address to get spendable UTXOs
        self.generate(self.nodes[0].generatetoaddress(101, addr_bech32), 101)
        self.sync_all()
        
        # Check balance
        balance = wallet.getbalances()["mine"]["trusted"]
        assert_equal(balance > 0, True, "Wallet should have balance after mining")
        self.log.info(f"Wallet balance: {balance}")
        
        # Get another bech32 address to send to
        addr_bech32_2 = wallet.getnewaddress("", "bech32")
        self.log.info(f"Generated second bech32 address: {addr_bech32_2}")
        
        # Send some coins (should succeed now)
        txid = wallet.sendtoaddress(addr_bech32_2, 1.0)
        self.log.info(f"Sent transaction: {txid}")
        
        # Verify transaction was created
        assert txid is not None, "Transaction should be created"
        ...
        assert_equal(tx_info["confirmations"] > 0, True, "Transaction should be confirmed")
        
        # Verify balance changed
        new_balance = wallet.getbalances()["mine"]["trusted"]
        self.log.info(f"New wallet balance: {new_balance}")
        assert_equal(new_balance < balance, True, "Balance should decrease after sending")
        
        # Also test with legacy P2PKH
        self.log.info("Test descriptor wallet signing with legacy P2PKH addresses")
        addr_legacy = wallet.getnewaddress("", "legacy")
        self.log.info(f"Generated legacy address: {addr_legacy}")
        
        # Send from bech32 to legacy (should succeed)
        txid2 = wallet.sendtoaddress(addr_legacy, 0.5)
        assert txid2 is not None, "Transaction from bech32 to legacy should be created"
        self.log.info(f"Sent transaction from bech32 to legacy: {txid2}")
